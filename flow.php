        ...
        if ($val > 0)
        {
            ...
            else
            {
                ...
                $goods_price =get_final_price($goods['goods_id'], $val, true, $attr_id);
 				...
 				$agent_level = get_agent_brand_level($goods['goods_id'], $_SESSION['user_id']);
 				
                $sql = "select price from " .  $ecs->table('huo15_gdextend') . " where goods_id = " . $goods['goods_id'] . " and level = " . $agent_level;
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0)
    {
       ...
       // 用户没有登录且没有选定匿名购物，转向到登录页面 
    	show_message('检测到您没有填写配送地址，为了安全起见，请您重新登陆！','登陆', 'flow.php?step=login');
    ...
    //huo15 商品等级价格
   $cart_goods = get_agent_priceAndMinNum($cart_goods, $cart_goods['goods_id'], $ecs, $db);
   
   //计算总价格 huo15
   $tmp = array();
   foreach ($cart_goods as $k => $v) {
   		$v['subtotal'] = floatval($v['agent_price']) * floatval($v['goods_number']);
   		$tmp[$k] = $v;
   }
   $cart_goods = $tmp;
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
    	//'agent_parent_id' => intval($agent_parent_id),
	...
	//添加商品等级价格
	//$cart_goods = get_agent_priceAndMinNum($cart_goods, $cart_goods['goods_id'], $ecs, $db);
	$sql = "select brand_id from " . $ecs->table('goods') . " where goods_id = " . $cart_goods[0]['goods_id'];
	$agent_brand_id = $db->getOne($sql);
	
	$sql = "select brand_name from " . $ecs->table('brand') . " where brand_id = " . $agent_brand_id;
    $agent_brand = $db->getOne($sql);
    //获取父级别代理商
    $agent_parent_id = get_parent_id($user_id, $agent_brand, $ecs, $db);
    
    if (!$agent_parent_id) {
    	$agent_parent_id = 0;
    }
    $order['agent_parent_id'] = intval($agent_parent_id);
   ...
   /* 判断是不是实体商品 */
    ...
    $sql = 'update ' .$GLOBALS['ecs']->table('goods') . ' AS a, ' .$GLOBALS['ecs']->table('cart') . ' AS b '.
    		'set a.sales_count = a.sales_count + b.goods_number '.
    		"WHERE a.goods_id = b.goods_id AND b.user_id = '".$_SESSION['user_id']."' AND b.rec_type = '$flow_type'";
...
else
{
    ...
    $cart_goods = get_cart_goods();
   	...
   	//获取混批起价
   // $agent_level = get_agent_brand_level($goods['goods_id'], $_SESSION['user_id']);	
   	$cart_goods['goods_list'] = get_agent_min_price($cart_goods['goods_list'], $_SESSION['user_id']);
   //获取混批总金额
   //获取所有品牌ID
   	$brands_agent = array();
   	foreach ($cart_goods['goods_list'] as $k => $v) {
   		$brands_agent[$k] = $v['brand_id'];
   	}
   	$brands_agent = array_unique($brands_agent);
    $smarty->assign('agent_brand_num', $brands_agent);
