  ...
  <div id="leftTabBox" class="loginBox">
    <div class="hd"> <span>欢迎光临，登录后体验更多服务</span>
      <ul>
        ...
        <li style="display:none;" <?php if ($this->_var['action'] == 'register'): ?> class="on"<?php endif; ?>><a href="javascript:void(0)">注册</a></li>
      </ul>
    </div>
    <div class="bd"<?php if ($this->_var['action'] == 'register'): ?> style="display:none"<?php endif; ?>>
      <ul>
        <div class="table_box">
		  ...
		  <dl>
              <dd>
               <label class="leftlabel"><select name="login_type"><option value="0">代理商用户名</option><select name="login_type"><option value="1">用户名</option><!--<option value="3">会员卡</option></select>--></label>
              </dd>
            </dl>
            <dl>
              <dd>
            ...
            <dl>
              <dd>
                <input type="checkbox" value="1" name="remember" id="remember" checked="checked" style="vertical-align:middle; zoom:200%;" /><label for="remember"> 一个月内免登录</label>
              </dd>
            </dl>
            <dl>
              <dd>
    ...
    <div class="bd"<?php if ($this->_var['action'] == 'login'): ?> style="display:none"<?php endif; ?>>
      <ul style="height:25rem">
        ...
        <?php $_from = $this->_var['extend_info_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['v']):
?>
        <?php if ($this->_var['v']['display']): ?>
        <dl>
         	<dd>
            	<?php if ($this->_var['v']['field_type'] == 0): ?>
            	<input placeholder="<?php echo $this->_var['v']['reg_field_name']; ?><?php if ($this->_var['v']['is_need']): ?>(必须填写)<?php endif; ?>" class="inputBg" name="extend_field<?php echo $this->_var['v']['id']; ?>" id="extend_field<?php echo $this->_var['v']['id']; ?>" type="text" />
                <?php elseif ($this->_var['v']['field_type'] == 1): ?>
                	性别<?php if ($this->_var['v']['is_need']): ?>(必须填写)<?php endif; ?>：
                    	<select name="extend_field<?php echo $this->_var['v']['id']; ?>">
                        	<option value="0">男</option>
                            <option value="1">女</option>
                        </select>
                    
                <?php elseif ($this->_var['v']['field_type'] == 2): ?>
       ...
       <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?> 
        ...
        <input type="hidden" name="apply_brand" value="<?php echo $this->_var['apply_brand']; ?>" />
        <input type="hidden" name="apply_level" value="<?php echo $this->_var['apply_level']; ?>" />
