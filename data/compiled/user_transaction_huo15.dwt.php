...
<link href="<?php echo $this->_var['ectouch_themes']; ?>/css/huo15ship.css" rel="stylesheet" type="text/css" />
...
?>
            ...
            <dl>
              ...
              <?php else: ?>
              <input  style="width: 3%;" type="radio" name="sex" value="0"  <?php if (! $this->_var['field']['content']): ?>checked<?php endif; ?> > 男<input       type="radio" style="width: 3%;" name="sex" value="1" <?php if ($this->_var['field']['content']): ?>checked<?php endif; ?>> 女 
              <?php endif; ?>
            </dl>
            <?php endif; ?>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
...
?>
    ...
    <?php if ($this->_var['action'] == 'trace'): ?>
    <script src="/mobile/themes/miqinew/js/huo15weixin.js"></script>
	<style>
	
	#huo15_goods_content tbody {
		width:100%;
    ...
    <div class="fullscreen">
      ...
      <div data-role="content" class="smart-result">
        <div class="content-primary" id="huo15_content_tb">
        <table id="huo15_goods_content" cellpadding="0" cellspacing="0">
       
        <thead class="alignCenter"></thead>
        <caption>二维码扫码操作等待中……</caption>
        <tbody>
          ...
          <tr>
          <td class="order_handler" id="gscan_btn"><span style="color:red">扫码</span></td>
          </tr>
           <tr>
          <td class="order_handler" id="gclear_btn"><span style="color:red">清除信息</span></td>
          </tr>
          <div id="traceContent">
...
<script>
<?php if ($this->_var['goodsNumber']): ?>
var goodsNumber = <?php echo $this->_var['goodsNumber']; ?>;
var shipedNumber = <?php echo $this->_var['shipedNumber']; ?>;
<?php endif; ?>
 wx.config({
   // debug: true,
	...
	$('#gclear_btn').click(function () {
		$('#traceContent').empty();
	});
	$('#gscan_btn').click(function () {
		...
		wx.scanQRCode({
			...
			success: function (res) {
				...
				$.post(
					'user.php?act=trace_do',
					{code : result},
					function (objStr) {
						
						var obj = {};
						var txt = '<tr class="cap"><td class="left_td">代理商</td><td class="right_td">下单时间</td></tr>';
						obj = eval(objStr);
						var p = /[\d]+/;
						for(x in obj) {
							if (p.test(x)) {
						...
						$('#traceContent').append(txt);
						$('#traceContent').find('td').css('border','1px solid #ccc');
    ...
    <?php if ($this->_var['huo15action'] == 'order_scan'): ?>
    ...
    <div class="fullscreen">
      ...
      <div data-role="content" class="smart-result">
        <div class="content-primary" id="huo15_content_tb">
        <table id="huo15_goods_content" cellpadding="0" cellspacing="0">
        <h4>订单商品数目：<span style="color:red"><?php echo $this->_var['goodsNumber']; ?></span>, &nbsp; &nbsp; &nbsp; &nbsp;已发货数目：<span style="color:red"><?php echo $this->_var['shipedNumber']; ?></span></h4>
        <thead class="alignCenter"></thead>
        <caption>二维码扫码操作等待中……</caption>
        <tbody>
          ...
          <table id="queryResult" cellpadding="0" cellspacing="0">
          <tr>
          <td class="order_handler" id="ship_btn"><span style="color:red">确认发货</span></td>
          </tr>
          <tr>
          <td class="order_handler" id="scan_btn"><span style="color:red">扫描</span></td>
          </tr>
           <tr>
          <td class="order_handler" id="clear_btn"><span style="color:red">清除条码</span></td>
          </tr>
          </table>
        </div>
      </div>
 ...
 wx.config({
	...
	$('#scan_btn').click(function () {
		...
		wx.scanQRCode({
			...
			success: function (res) {
				//添加点击事件
				//$('#ship_btn').bind('click', ship_btn_fn);
				var result = res.resultStr; // 当needResult 为 1 时，扫码返回的结果
				if (result) {
					//添加二维码
					if($('.huo15No').last().text() != result) {
						if (result.substr(0,1) == 'C') {
							$('#huo15_goods_content').find('tbody').append(
								'<tr><td class="huo15NoC huo15No">' + result + '</td><td class="del_huo15_no" onclick="delThis(this)">(箱码)删除</td></tr>'
							);
						
						} else {
							$('#huo15_goods_content').find('tbody').append(
								'<tr><td class="huo15No">' + result + '</td><td class="del_huo15_no" onclick="delThis(this)">删除</td></tr>'
							);
						}
					}
					...
					//修改提示信息
					$('#huo15_goods_content').find('caption').html(
						'二维码<span class="no_huo15">' + result + '</span>录入成功，请继续操作'
					);
					
					//清除条码
					$('#clear_btn').click(function () {
						$('#huo15_goods_content').find('tbody').empty();
						//修改提示信息
						$('#huo15_goods_content').find('caption').html(
							'<span class="no_huo15">发货处理待命中</span>'
						);
					});
...
function ship_btn_fn() {
	var r = confirm("确定发货？");
	if (r) {
		//发送数据
		var data = {};
		$('.huo15No').each(function(index, element) {
			data['huo15No' + index] = $(element).text();
		});
		<?php if ($this->_var['huo15_order_id']): ?>
		data.order_id = <?php echo $this->_var['huo15_order_id']; ?>;
		<?php endif; ?>
		//alert($('.huo15No').length);
		//按钮失效
		//$('#ship_btn').unbind('click');
		
		//发货请求
		$.post(
			"huo15user.php?act=huo15_shipping",
			data,
			function (res) {
				alert(res);
				if( res.length > 5) {
				...
				if( res.length <= 5) {
					...
					//修改标题
					$('#huo15_content_tb').find('h4').css('display', 'none');
					$('#huo15_goods_content').find('thead').find('span').remove();
					$('#huo15_goods_content').find('thead').html('<span style="color:red">发货数目已经足够,或者箱码数量超过发货数目</span><thead>');
				} else if (res.count) {
					//修改提示信息
					$('#huo15_goods_content').find('caption').html(
						'<span class="no_huo15">成功发货数目：<font color="black">' + res.count + '</font></span>'
					);
					//修改标题
					$('#huo15_content_tb').find('h4').css('display', 'none');
					$('#huo15_goods_content').find('thead').find('span').remove();
					$('#huo15_goods_content').find('thead').html(
						'订单商品数目：<span style="color:red">'+res.goodsNumber+'</span>, &nbsp; &nbsp; &nbsp; &nbsp;已发货数目：<span style="color:red">'+res.shipedNumber+'</span>'
					);
					...
					$('#huo15_content_tb').find('h4').css('display', 'none');
					$('#huo15_goods_content').find('thead').find('span').remove();
					$('#huo15_goods_content').find('thead').html('<span style="color:red">发货数目已经足够或者箱码数量超过发货数量</span><thead>');
				}
...
function delThis(td) {
    ...
    <?php if ($this->_var['action'] == iuser_order_detail): ?>
    ...
    <section class="wrap">
      <section class="order_box padd1 radius10">
        <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
         <form action="user.php?act=operate" method="post" id="agent_order_operate">
      	<tr>
...
?> 
        ...
        <tr>
            <td><a href="user.php?act=order_tracking&order_id=<?php echo $this->_var['order']['order_id']; ?>" class="c-btn3">进入发货状态</a></td>
        </tr>
        <?php endif; ?>
      </table>
      </section>
      ...
      <?php if ($this->_var['order']['postscript']): ?>
         <tr>
          <td width="32%" align="right">订单附言：</td>
        	 <td width="68%" align="left"><?php echo $this->_var['order']['postscript']; ?></td>
        </tr>
         
          <?php endif; ?>
        <tr>
          <td width="32%" align="right">客户姓名：</td>
          <td width="68%" align="left"><?php echo $this->_var['order']['consignee']; ?></td>
        </tr>
       
        <?php if ($this->_var['order']['exist_real_goods']): ?>
        <tr>
          <td align="right">配送详细地址：</td>
...
?>
          ...
          <tr>
            <td style="border-bottom:1px #CCCCCC dashed">数量：<?php echo $this->_var['goods']['goods_number']; ?><br></td>
          </tr>
          <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
      </table>
      <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table" >
        <tr>
           ...
           <script>
           $(function () {
				$('#goods_amount_btn').click(function () {
					
					$.post(
						'user.php?act=updat_order_amount',
						{
							order_amount : $('#goods_amount_huo15').val(),
							order_id : '<?php echo $this->_var['order']['order_id']; ?>'
						},
						 function (res) {
							 res = eval('res = ' + res);
							 $('#goods_amount_huo15').val(res.amount);
							 $('#goods_amount_label').find('span').text(res.amount);
							alert(res.msg);
						}
					);
				});   
  ...
  <td colspan="5"><textarea name="action_note" cols="30" rows="3"></textarea></td>
        </tr>
...
?>
    ...
    <?php if ($this->_var['action'] == agent_order_detail): ?>
    ...
    <section class="wrap">
      <section class="order_box padd1 radius10">
        ...
        <tr>
        	<td id="goods_amount_label">订单金额：<?php echo $this->_var['order']['formated_total_fee']; ?></td>
        </tr>
        <tr>
        	<td>下单时间：<?php echo $this->_var['order']['formated_add_time']; ?></td>
        </tr>
...
?>
      ...
      <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table" >
        <tr>
          <td align="right"> <?php echo $this->_var['lang']['goods_all_price']; ?><?php if ($this->_var['order']['extension_code'] == "group_buy"): ?><?php echo $this->_var['lang']['gb_deposit']; ?><?php endif; ?>: <input type="text" id="goods_amount_huo15" value="<?php echo $this->_var['order']['goods_amount']; ?>" /><input id="goods_amount_btn" type="button" value="修改" />
           </td>
           <script>
           $(function () {
				$('#goods_amount_btn').click(function () {
  ...
  <td colspan="5"><textarea name="action_note" cols="30" rows="3"></textarea></td>
   ...
   <?php if ($this->_var['operable_list']['pay']): ?><?php else: ?><input name="invoice_no" type="text" style="color:#00F" value="<?php echo $this->_var['order']['order_sn']; ?>" /><input name="to_shipping" type="submit" style="color:#F00" value="快速发货" class="button"/><?php endif; ?>
  
    
    <?php if ($this->_var['operable_list']['confirm']): ?>
      <input name="confirm" type="submit" value="审核订单" class="button" />
        <?php endif; ?>
         <?php if ($this->_var['operable_list']['pay']): ?>
        <input name="pay" type="submit" value="付款" class="button" />
        <?php endif; ?>
         <?php if ($this->_var['operable_list']['unpay']): ?>
        <input name="unpay" type="submit" class="button" value="未付款" />
        <?php endif; ?>
        ...
        <input name="unship" type="submit" value="设置为未发货" class="button" />
        <?php endif; ?>
         <?php if ($this->_var['operable_list']['receive']): ?>
       <?php echo $this->_var['lang']['op_receive']; ?>
        <?php endif; ?> 
       ...
       <?php echo $this->_var['lang']['op_return']; ?>
        <?php endif; ?>
        ...
        <?php endif; ?>
         <!--<input name="after_service" type="submit" value="售后" class="button" />--><?php if ($this->_var['operable_list']['remove']): ?>
        <input name="remove" type="submit" value="删除订单" class="button" onClick="return window.confirm('您确定要删除该订单吗？');" />
        <?php endif; ?>
...
?>
        ...
        <input name="order_id" type="hidden" value="<?php echo $_REQUEST['order_id']; ?>"></td>
       	...
       	<!--<td class="order_handler"><span style="color:red">已确认</span></td><td class="order_handler"><span style="color:red">已确认</span></td><td class="order_handler"><span style="color:red">已确认</span></td>-->
       </tr>
     </form>
        <?php if ($this->_var['allow_edit_surplus']): ?>
        <tr>
...
?>
    ...
    <?php if ($this->_var['action'] == 'agent_add_order'): ?>
    <header id="header">
    ...
    <section class="wrap order_list">
      ...
      <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
      	<tr>
        	<td class="order_status" style="border-bottom:1px #CCCCCC dashed">新订单号预设：<input type="text" value="<?php echo $this->_var['order_sn']; ?>" name="order_sn" disabled="disabled" /></td>
        </tr>
        <tr>
        	<td class="order_status" style="border-bottom:1px #CCCCCC dashed">下级代理商：
           	 <select name="user_id">
          		<?php $_from = $this->_var['agent_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['v']):
?>
             	<option value="<?php echo $this->_var['v']['user_id']; ?>" <?php if ($this->_var['v']['user_id'] == 'n'): ?>selected = "selected"<?php endif; ?>><?php echo $this->_var['v']['user_name']; ?></option>
                <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
             </select>
            </td>
        </tr>
        <tr>
        	<td class="order_content">选择收货地址：<input type="button" value="重选" id="reset_btn" />
        ...
        <tr >
        	<td class="order_content">备注：<textarea id="remark" cols="40" rows="5"></textarea></td>
        </tr>
        <tr class="goods_area">
        	<td class="add_goods">添加商品(+)</td>
        </tr>
         <tr class="goods_area goods_list">
            <td class="mygoods">
            <select class="choose_goods">
            	
                <?php $_from = $this->_var['goods_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['v']):
?>
                	<?php $_from = $this->_var['v']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'val');if (count($_from)):
    foreach ($_from AS $this->_var['val']):
?>
                    	<option value="<?php echo $this->_var['val']['goods_id']; ?>"><?php echo $this->_var['val']['goods_name']; ?></option>
                    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
            </select>
            &nbsp;&nbsp; 数量：<input type="text" class="good_num" value="1" />&nbsp;&nbsp; <input type="button" class="del_this" value="删除" /></td>   
        </tr>
        <tr>
        	<td class="order_content"><input type="button" value="添加订单" id="add_order_btn" /></td>
        </tr>
        <tr>
    ...
    <a href="javascript:;" style="text-align:center" class="get_more">代理商管理系统</a>
    <input type="hidden" id="session_id" value="<?php echo $this->_var['session_id']; ?>" />
    </section>
	<style>
		/* 火一五信息科技 代理商系统添加订单页面*/
		.consignee {
    ...
    <script type="text/javascript">
    	$(function () {
			...
			$('.goods_area').hide();
			//下级代理商选择
			var user_id = 0;//初始化下级代理商ID
			$('[name="user_id"]').change(function () {
				
				user_id = $(this).val();
				$.post(
					'user.php?act=agent_add_order',
					{
						"user_id" : user_id
					},
					function (res) {
						...
						for ( n in res.result) {
							...
							//收货地址列表
							if (res.result[n]['consignee']) {
								
								txt += '<ul class="consignee" id="c'+n+'"><li class="displayNone"><em id="c_name">'+res.result[n]['consignee']+'</em><em id="c_tel">'+res.result[n]['tel']+'</em><em id="c_country">'+res.result[n]['country']+'</em><em id="c_province">'+res.result[n]['province']+'</em><em id="c_city">'+res.result[n]['city']+'</em><em id="c_district">'+res.result[n]['district']+'</em><em id="c_address">'+res.result[n]['address']+'</em></li><li>姓名：'+res.result[n]['consignee']+'</li><li>手机：'+res.result[n]['tel']+'</li><li>地址：'+res.result[n]['country_name'] + res.result[n]['province_name']+ res.result[n]['city_name'] + res.result[n]['district_name'] + res.result[n]['address'] +'</li></ul>';
							}
						}
						
						$('#address_default').html(
							txt
						);
			...
			$('#add_order_btn').click(function () {
				//检测是否合法
				var r = confirm('确定添加订单？');
				if (r) {
					//检测是否填写了备注
					var flag = 1;
					
					if ($('#remark').val() != '') flag = 0;
					//检测是否选择了地址
					if ($('.consignee').length != 1 && flag) {
						alert('请选择收货地址, 或者填写下备注');
						return false;
					}
					...
					var data = {
						...
						'user_id' : user_id,
						'consignee' : $('#c_name').text(),
						'country' : $('#c_country').text(),
						'province' : $('#c_province').text(),
						'city' : $('#c_city').text(),
						'district' : $('#c_district').text(),
						'address' : $('#c_address').text(),
						'tel' : $('#c_tel').text(),
						'session_id' : $('#session_id').val(),
						'postscript' : $('#remark').val()
						
					};
					
					$.post(
						'user.php?act=agent_add_order_do',
						data,
						function (re) {
							...
							if (re.result > 0) {
								...
								//修改字体
								$('#add_order_btn').val('提交订单');
								$('#add_order_btn').unbind('click');
								$('#add_order_btn').click(function () {
									var data = {};
									
									$('.mygoods').each(function(index, element) {
										data['good' + index] = {};
										data['good' + index]['goods_id'] = $(element).find('.choose_goods').val();
										data['good' + index]['goods_name'] =  $(element).find('.choose_goods').find("option:selected").text();
										data['good' + index]['order_sn'] =  $('[name="order_sn"]').val();
										data['good' + index]['goods_num'] =  $(element).find('input.good_num').val();
										data['good' + index]['user_id'] =  user_id;
                                       
                                    });
									//添加到订单商品
									$.post(
										'user.php?act=add_goods_done',
										data,
										function(r) {
											//alert(r.result);
											alert(r.err_msg);
											if (r.result == 1) {
												window.location = 'user.php?act=order_tracking&order_id=' + r.order_id;
											}
										},
										'json'
									);
			...
			$('.add_goods').click(function () {
				...
				$(' <tr class="goods_area goods_list">'+clone.html()+'</tr>').insertAfter($(this).parent());
				//删除商品
			$('.del_this').each(function(index, element) {
                $(element).click(function () {
					$(this).unbind('click');
    ...
    <?php if ($this->_var['action'] == 'user_add_order'): ?>
    <script src="data/static/js/region.js"></script>
    <header id="header">
    ...
    <section class="wrap order_list">
      ...
      <table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table">
      	<tr>
        	<td class="order_status" style="border-bottom:1px #CCCCCC dashed">新订单号预设：<input class="inputBg" type="text" value="<?php echo $this->_var['order_sn']; ?>" name="order_sn" disabled="disabled" /></td>
        </tr>
        <tr>
        	<td class="order_status" style="border-bottom:1px #CCCCCC dashed">用户微信号：<input class="inputBg" type="text" value="" name="wx" /></td>
        </tr>
        <tr>
...
?>
    ...
    <a href="javascript:;" style="text-align:center" class="get_more">美释品牌代理商管理系统</a>
    <input type="hidden" id="session_id" value="<?php echo $this->_var['session_id']; ?>" />
    </section>
	<style>
		/* 火一五信息科技 代理商系统添加订单页面*/
    ...
    <script type="text/javascript">
    	$(function () {
			...
			$('#add_order_btn').click(function () {
				var data = {};
				data.order_sn = $('[name="order_sn"]').val();
				data.wx = $('[name="wx"]').val();
				data.consignee = $('#consignee_0').val();
				data.mobile = $('#tel_0').val();
				data.country = $('#selCountries_0').val();
				data.province = $('#selProvinces_0').val();
				data.city = $('#selCities_0').val();
				data.district = $('#selDistricts_0').val();
				data.address = $('#address_0').val();
				data.postscript = $('#remark').val();
				//发送到服务器，保存到数据库
				$.post(
					'huo15user.php?act=user_add_order',
					data,
					function (res) {
						...
						if (res.result > 0) {
							...
							$('#add_order_btn').click(function () {
								...
								//添加到订单商品
								$.post(
									'huo15user.php?act=add_goods_done',
									data,
									function(r) {
										//alert(r.result);
										alert(r.err_msg);
										if (r.result == 1) {
											window.location = 'huo15user.php?act=order_scan&order_id=' + r.order_id;
										}
									},
									'json'
								);
    ...
    <?php if ($this->_var['action'] == 'agent_order_list'): ?>
    <header id="header">
...
?>
    ...
    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    jQuery(function($){
    	$('.order_list').more({'address': 'user.php?act=agent_async_order_list&flag=flag:<?php echo $this->_var['flag']; ?>', amount: 3, 'spinner_code':'<div style="text-align:center; margin:10px;"><img src="<?php echo $this->_var['ectouch_themes']; ?>/images/loader.gif" /></div>'})
    ...
    <?php if ($this->_var['action'] == 'huo15_order_list'): ?>
    <header id="header">
      <div class="header_l header_return"> <a class="ico_10" href="user.php"> 返回 </a> </div>
      <h1> <?php echo $this->_var['lang']['label_order']; ?> </h1>
    </header>
...
?>
    ...
    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    jQuery(function($){
    	$('.order_list').more({'address': 'huo15user.php?act=agent_async_order_list&flag=flag:<?php echo $this->_var['flag']; ?>', amount: 3, 'spinner_code':'<div style="text-align:center; margin:10px;"><img src="<?php echo $this->_var['ectouch_themes']; ?>/images/loader.gif" /></div>'})
		$(window).scroll(function () {
			if ($(window).scrollTop() == $(document).height() - $(window).height()) {
				$('.get_more').click();
			}
