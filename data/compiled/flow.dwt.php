...
<script type="text/javascript" src="<?php echo $this->_var['ectouch_themes']; ?>/js/jquery-huo15.js"></script>
...
?> 
        ...
        <div class="desc new-tbl-cell"> <a style="color:#333" href="goods.php?id=<?php echo $this->_var['goods']['goods_id']; ?>" target="_blank" class="fragment">
          ...
          <div class="b">
          <span class="hpqj" style="display:none"><?php echo $this->_var['goods']['min_num']; ?></span>
          <span class="good_price_agent" style="display:none"><?php echo $this->_var['goods']['goods_price']; ?></span>
          <span class="good_brand_id" style="display:none"><?php echo $this->_var['goods']['brand_id']; ?></span>
           <span class="good_brand_name" style="display:none"><?php echo $this->_var['goods']['brand_name']; ?></span>
          
          <span style="margin:1rem"><span class="huo15_pp<?php echo $this->_var['goods']['brand_id']; ?> displayNone"><?php echo $this->_var['goods']['brand_name']; ?>混批起价：<?php echo $this->_var['goods']['min_num']; ?>元</span></span> <em class="price"><?php echo $this->_var['goods']['goods_price']; ?>×</em> <span style="jianju"></span> <span class="txt"> 
    ...
    <ul class="radius10 itemlist">
    	<li style="display:none">商品总数量：<em class="price"><?php echo $this->_var['total']['total_number']; ?></em></li>
        <li style="display:none">商品总金额：<em class="price"><?php echo $this->_var['total']['goods_price']; ?></em></li>
         <li class="li_yqts">友情提示：<em class="yqts"></em></li>
    </ul>
  </form>
...
<footer class="toolbar">
  <p style="display:none">合计: <em class="price" id="goods_subtotal" ><?php echo $this->_var['total']['goods_price']; ?></em></p>
  <a href="javascript:void(0)" onClick="checkMinNum(this)">结算(<em class="num" id="total_number"><?php echo $this->_var['total']['total_number']; ?></em>)</a> </footer>
<?php else: ?>
<section class="wrap"  >
...
<section class="wrap">
  <form action="flow.php" method="post" name="theForm" id="theForm" onSubmit="return checkOrderForm(this)">
    ...
    <section class="order_box padd1 radius10" style="padding-top:0.3rem;padding-bottom:0.3rem;">
      <div class="table_box table_box2" style="display:none;"> 
...
?>
		...
		<div class="dl_box" id="inviype_box" style="margin-bottom:0.5rem; ">
    ...
    <div class="blank3"> </div>
    <section class="order_box padd1 radius10" style="padding-top:0.3rem;padding-bottom:0.3rem;">
      <div class="table_box table_box2" id="liuyan" style=" margin-bottom:0.5rem;"> 
...
?>
		...
		<div class="dl_box">
         ...
         <dl>
         	<dd>
            	<textarea placeholder="请输入订单备注" name="postscript" rows="3" cols="40">
...
?>
        <dl>
          ...
          <dd class="dd3 w30"> ￥<?php echo $this->_var['goods']['agent_price']; ?> </dd>
        </dl>
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?> 
      ...
      <div id="J_allGoods">
        <div class="cont">
          <section class="order on">
            <h6 style="text-align:center;line-height:20px;">您的订单号: <font style="color:red"><?php echo $this->_var['order']['order_sn']; ?></font></h6>
            <table width="90%" align="center" border="0" cellpadding="15" cellspacing="0" style="border:1px solid #ddd; margin:10px auto;" bgcolor="#FFFFFF">
              <tr>
                <td align="left" style="padding: 5px;line-height: 24px;"><br><?php echo $this->_var['lang']['order_amount']; ?>: <strong><?php echo $this->_var['total']['goods_price_formated']; ?></strong><br></td>
              </tr>
...
var brandHuo15 = [];
<?php $_from = $this->_var['agent_brand_num']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['k'] => $this->_var['v']):
?>
	brandHuo15.push(<?php echo $this->_var['v']; ?>);
<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
var brandTotal = {};
for(i=0;i<brandHuo15.length;i++) {
	brandTotal['a' + i] = {};
	brandTotal['a' + i]['name'] = '';
	brandTotal['a' + i]['total'] = 0;
	brandTotal['a' + i]['uptotal'] = 0;
	
}
$(function () {
	
	$('#shipping_5').prop('checked', true);
	$('#payment_4').prop('checked', true);
	
	//获取每一个商品的信
	$('.new-tbl-type').each(function(index, element) {
        var hpqj = $(element).find('.hpqj').text();
		var p = $(element).find('.good_price_agent').text();
		var p = p.match(/[\d]+/);
		var n =  $(element).find('.num').val();
		var total = n * p;
		var brand_id = $(element).find('.good_brand_id').text();
		var brand_name = $(element).find('.good_brand_name').text();
		for(i=0;i<brandHuo15.length;i++) {
			if (brand_id == brandHuo15[i]) {
				brandTotal['a' + i]['name'] = brand_name;
				brandTotal['a' + i]['total'] = brandTotal['a' + i]['total'] + total;
				brandTotal['a' + i]['uptotal'] = hpqj;
			}
		}
		
			
		//alert(hpqj + ',' + p + ',' + n + ',' + total);
    });
	var txt = '';
	for(i=0;i<brandHuo15.length;i++) {
		txt = txt + "品牌\"" + brandTotal['a' + i]['name'] + "\"订单总金额：" + brandTotal['a' + i]['total'] + "元; 混批最低价：" + brandTotal['a' + i]['uptotal'] + "元<br/>";
	}
...
//获取每一个商品的信
	...
	for(i=0;i<brandHuo15.length;i++) {
		if (brandTotal['a' + i]['total'] >= brandTotal['a' + i]['uptotal']) {
			//alert(brandTotal['a' + i]['total'] +','+ brandTotal['a' + i]['uptotal']);
			//return false;
			window.location = "flow.php?step=checkout";
			} else {
				window.location = "flow.php?step=checkout";
				//alert('购买量不达标，请查看“友情提示”');
				return false;
			}
	}
