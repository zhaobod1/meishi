 ...
 * @return      array
 */
function get_regions_name($regin_id)
{
	$sql = 'SELECT region_name FROM ' . $GLOBALS['ecs']->table('region') .
	" WHERE region_id = " .$regin_id;
 ...
 * @return array $arr
 */
function get_parent_agent($user_id,$ecs,$db) {
	$sql = 'SELECT `parent_id`, `agent_level` FROM ' . $ecs->table('huo15_agent').' WHERE user_id = '.$user_id;
	$arr = $db->getRow($sql);
	return $arr;
}
 ...
 * @return int $arr
 */
function get_parent_level($user_id,$ecs,$db) {
	$sql = 'SELECT `agent_level` FROM ' . $ecs->table('huo15_agent').' WHERE user_id = '.$user_id;
	$arr = $db->getOne($sql);
	return intval($arr);
}
/**
 ...
 * @return int $arr
 */
function get_parent_id($user_id,$brand,$ecs,$db) {
	$sql = 'SELECT `parent_id` FROM ' . $ecs->table('huo15_agent').' WHERE user_id = '.$user_id . " and agent_brand = '" . $brand . "'";
	$arr = $db->getOne($sql);
	return intval($arr);
}
/**
 *  火一五信息科技 验证是否是真代理
 */
function vertify_agent($level_parent_id, $ecs, $db) {
	if ($level_parent_id == 0) {
		return true;
	}
	$sql = 'SELECT `user_id` FROM '.$ecs->table('users') . ' WHERE `user_id` = '. $level_parent_id;
 ...
 * @return array:
 */
function get_dealer_level() {
	$dealer_level = unserialize($GLOBALS['_CFG']['dealer_level']);
	empty($dealer_level) && $dealer_level = array();
	return $dealer_level;
}
 ...
 * @param unknown $agent_level
 */
function agent_priceAndMinNum($goods, $user_id, $ecs, $db) {
	...
	if (!isset($_SESSION['user_id'])) {
		show_message('请先登录！', '代理商入口', 'user.php');
	}
	// 等级level ， 商品ID，goods_id
	//查看代理商的代理信息：代理等级agent_level，代理品牌agent_brand， 品牌ID：brand_id
	$sql = "SELECT agent_brand, agent_level FROM " . $ecs->table('huo15_agent') . " WHERE user_id = " . $user_id . " AND is_ok = 1 ";
	$level_info = $db->getAll($sql);
	...
	foreach ($level_info as $k => $v) {
		$sql = "select brand_id from " . $ecs->table('brand') . " where brand_name='" . $v['agent_brand'] . "'";
		$v['brand_id'] = $db->getOne($sql);
		$temp[$k] = $v;
	}
	$level_info = $temp;
	...
	foreach ($goods as $key => $g) {
		...
		if (isset($g['goods_id'])) {
			...
			$sql = "select brand_id from " . $ecs->table('goods') . " where goods_id = " . $g['goods_id'];
			
			$g['brand_id'] = $db->getOne($sql);
			
			//赋值商品代理等级
			foreach ($level_info as $v) {
					
				if ($v['brand_id'] == $g['brand_id']) {
					$g['agent_level'] = $v['agent_level'];
				}
			}
				
				
			if (isset($g['agent_level'])) {
				$gg = $g;
				//获取代理价格和启货量

				$sql = "select min_num, price from " . $ecs->table('huo15_gdextend') . " where goods_id = " . $gg['goods_id'] . " and level=" . $gg['agent_level'];
				
				if (!$db->query($sql)) {
					show_message('请先登录系统！');
				}
				$tmp = $db->getRow($sql);
				$gg['agent_price'] = $tmp['price'];
				$gg['min_num'] = $tmp['min_num'];
				if ($gg['goods_number']) {
					$gg['agent_total'] = floatval($gg['goods_number']) * floatval($gg['agent_price']);
				}
 ...
 * @param unknown $agent_level
 */
function get_agent_priceAndMinNum($goods, $goods_id, $ecs, $db) {
	...
	// 等级level ， 商品ID，goods_id
	//查看代理商的代理信息：代理等级agent_level，代理品牌agent_brand， 品牌ID：brand_id
	$sql = "SELECT agent_brand, agent_level FROM " . $ecs->table('huo15_agent') . " WHERE user_id = " . $_SESSION['user_id'] . " AND is_ok = 1 ";
	$level_info = $db->getAll($sql);
...
/**
 * 通过商品ID 结合用户ID，查询到商品等级
 * @param unknown $goods_id
 * @param unknown $user_id
 * @return unknown
 */

function get_agent_brand_level($goods_id, $user_id) {
	$sql = "select brand_id from " . $GLOBALS['ecs']->table('goods') . " where goods_id = " . $goods_id;
	$brand_id = $GLOBALS['db']->getOne($sql);
	$sql = "select brand_name from" .$GLOBALS['ecs']->table('brand') . " where brand_id = " . $brand_id;
	$agent_brand = $GLOBALS['db']->getOne($sql);
	$sql = "select agent_level from ".$GLOBALS['ecs']->table('huo15_agent') . " where user_id = " . $user_id . " and agent_brand = '".$agent_brand . "'";
	$agent_level = $GLOBALS['db']->getOne($sql);
	return $agent_level;
}
/**
 * 获取商品品牌名称
 ...
 * @return unknown
 */
function get_agent_brand_name($goods_id, $user_id) {
	$sql = "select brand_id from " . $GLOBALS['ecs']->table('goods') . " where goods_id = " . $goods_id;
	$brand_id = $GLOBALS['db']->getOne($sql);
	$sql = "select brand_name from" .$GLOBALS['ecs']->table('brand') . " where brand_id = " . $brand_id;
	$agent_brand = $GLOBALS['db']->getOne($sql);
 ...
 * @return unknown
 */
function get_agent_brand_id($goods_id, $user_id) {
	$sql = "select brand_id from " . $GLOBALS['ecs']->table('goods') . " where goods_id = " . $goods_id;
	$brand_id = $GLOBALS['db']->getOne($sql);
 ...
 */
function get_agent_min_price($goods, $session_id) {
	$t = array();
	foreach ($goods as $k => $v) {
		//获取代理商对应商品的级别
		$v['agent_level'] = get_agent_brand_level($v['goods_id'], $session_id);
		//获取商品最小混批价格
		$sql = "select min_num from "  . $GLOBALS['ecs']->table('huo15_gdextend') . " where level = " . $v['agent_level'] . " and goods_id = " . $v['goods_id'];
		$v['min_num'] = $GLOBALS['db']->getOne($sql);
		$v['brand_name'] = get_agent_brand_name($v['goods_id'], $session_id);
		$v['brand_id'] =  get_agent_brand_id($v['goods_id'], $session_id);
		$t[$k] = $v;
	}
	return $t;
}
