...
function order_info($order_id, $order_sn = '')
{
    ...
    $total_fee = " (goods_amount) AS total_fee ";
 ...
 * @return  array   订单信息（金额都有相应格式化的字段，前缀是formated_）
 */
function iuser_order_info($order_id, $order_sn = '')
{
	/* 计算订单各种费用之和的语句 */
	//$total_fee = " (goods_amount - discount + tax + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee) AS total_fee ";
	...
	if ($order_id > 0)
	{
		$sql = "SELECT *, " . $total_fee . " FROM " . $GLOBALS['ecs']->table('huo15_order_info') .
		" WHERE order_id = '$order_id'";
	}
	else
	{
...
function order_goods($order_id)
{
	...
	//$goods_list = get_agent_priceAndMinNum($goods_list,'',$GLOBALS['ecs'], $GLOBALS['db']);
    //return $GLOBALS['db']->getAll($sql);
    return $goods_list;
}
/**
 ...
 * @return  array   订单商品数组
 */
function iuser_order_goods($order_id)
{
	$sql = "SELECT og.rec_id, og.goods_id, og.goods_name,   og.goods_number, " .
			"og.goods_price,  og.is_real, og.parent_id,  " .
			"og.goods_price * og.goods_number AS subtotal, g.goods_thumb " .
			"FROM " . $GLOBALS['ecs']->table('huo15_order_goods') . " as og left join " .$GLOBALS['ecs']->table('goods'). " g on og.goods_id = g.goods_id" .
			" WHERE og.order_id = '$order_id'";
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_id, g.goods_name, g.brand_id, g.goods_sn, g.is_on_sale, g.is_real, ".
    ...
    $sql = "select agent_level from " . $GLOBALS['ecs']->table('huo15_agent') . " where agent_brand='" . $brand_name . "' and user_id = " . $_SESSION['user_id'];
    $agent_level = $GLOBALS['db']->getOne($sql);
    $sql = "select price, min_num from " . $GLOBALS['ecs']->table('huo15_gdextend') . " where level = " . $agent_level . " and goods_id = " . $goods['goods_id'];
   
    $qpm =  $GLOBALS['db']->getRow($sql);
    $goods['agent_price'] = $qpm['price'];
    $goods['min_num'] = $qpm['min_num'];
   	$goods['goods_price'] =  $goods['agent_price'];
    if (empty($goods))
    {
    ...
    if ($num > 0)
    {
        ...
        {
            $goods_price = $goods['agent_price'];
            $parent['goods_price']  = max($goods[agent_price], 0);
...
/**
 * 查询购物车（订单id为0）或订单中是否有实体商品 垂直业务
 * @param   int     $order_id   订单id
 * @param   int     $flow_type  购物流程类型
 * @return  bool
 */
function user_exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{
	if ($order_id <= 0)
	{
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('cart') .
	...
	{
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('huo15_order_goods') .
		" WHERE order_id = '$order_id' AND is_real = 1";
	}
...
function agent_to_shipping($invoice_no, $order_id, $action_note, $ecs, $db) {
	...
	if (!empty($invoice_no))
	{
		...
		/* 查询：取得区域名 */
		$sql = "SELECT agency_id FROM " . $ecs->table('admin_user') . " WHERE user_id = '1'";
