...
function get_user_order_detail($order_id) {
	...
	$order = iuser_order_info($order_id);
	/* 获取订单中实体商品数量 */
	$order['exist_real_goods'] = user_exist_real_goods($order_id);
	/* 如果是未付款状态，生成支付按钮 */
	if ($order['pay_status'] == PS_UNPAYED &&
 ...
 * @return   arr        $order          订单所有信息的数组
 */
function get_agent_order_detail($order_id, $user_id = 0)
{
	include_once(ROOT_PATH . 'include/lib_order.php');
	...
	//检查订单是否属于该用户
	if ($user_id > 0 && $user_id != $order['agent_parent_id'])
	{
		$GLOBALS['err']->add($GLOBALS['_LANG']['no_priv']);
 ...
 * @return  bool        $bool
 */
function agent_affirm($order_id, $user_id = 0)
{
	/* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status FROM " .$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
    $order = $GLOBALS['db']->GetRow($sql);
    ...
    //将用户订单设置为取消
    $sql = "UPDATE ".$GLOBALS['ecs']->table('order_info') ." SET order_status = '".OS_CONFIRMED."' WHERE order_id = '$order_id'";
    if ($GLOBALS['db']->query($sql))
    {
        /* 记录log */
        order_action($order['order_sn'], OS_CONFIRMED, $order['shipping_status'], PS_UNPAYED, $user_id.'代理商确认订单','buyer');
        /* 退货用户余额、积分、红包 */
        if ($order['user_id'] > 0 && $order['surplus'] > 0)
        {
            $change_desc = sprintf($GLOBALS['_LANG']['return_surplus_on_cancel'], $order['order_sn']);
 ...
 * @return  array       $order_list     订单列表
 */
function get_agent_orders($user_id, $num = 10, $start = 0)
{
	/* 取得订单列表 */
	$arr    = array();
/*
	$sql = "SELECT order_id, order_sn, order_status, shipping_id, shipping_status, pay_status, add_time, " .
			...
			" WHERE user_id = '$user_id' ORDER BY add_time DESC";
			*/
	$sql = "SELECT agent_parent_id, order_id, order_sn, order_status, shipping_id, shipping_status, pay_status, add_time, " .
			"(goods_amount) AS total_fee ".
			" FROM " .$GLOBALS['ecs']->table('order_info') .
			" WHERE agent_parent_id = '$user_id' ORDER BY add_time DESC";
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		if ($row['order_status'] == OS_UNCONFIRMED)
		{
			$row['handler'] = "<a href=\"user.php?act=agent_affirm&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('确认要审核通过该订单吗？')) return false;\">审核订单</a>";
		}
		else if ($row['order_status'] == OS_SPLITED)
		{
			/* 对配送状态的处理 */
			...
			else
			{
				if ($row['pay_status'] == PS_UNPAYED)
				{
					@$row['handler'] = "<a href=\"user.php?act=agent_order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
				}
				else
				{
					@$row['handler'] = "<a href=\"user.php?act=agent_order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
				}
		...
		$arr[] = array('order_id'       => $row['order_id'],
				...
				'total_fee'      => price_format($row['total_fee'], false),
				'handler'        => $row['handler'],
				'agent_parent_id' => $row['agent_parent_id']
		);
	}
...
/**
 *  huo15 获取垂直业务指定范围的订单列表
 *
 * @access  public
 * @param   int         $user_id        代理商ID号
 * @param   int         $num            列表最大数量
 * @param   int         $start          列表起始位置
 * @return  array       $order_list     订单列表
 */
function get_iuser_orders($user_id, $num = 10, $start = 0)
{
	/* 取得订单列表 */
	$arr    = array();
	...
	$sql = "SELECT agent_parent_id, order_id, order_sn, order_status, shipping_id, shipping_status, pay_status, add_time, " .
			"(goods_amount) AS total_fee ".
			" FROM " .$GLOBALS['ecs']->table('huo15_order_info') .
			" WHERE agent_parent_id = '$user_id' ORDER BY add_time DESC";
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		else if ($row['order_status'] == OS_SPLITED)
		{
			...
			else
			{
				if ($row['pay_status'] == PS_UNPAYED)
				{
					@$row['handler'] = "<a href=\"huo15user.php?act=user_order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
				}
				else
				{
					@$row['handler'] = "<a href=\"huo15user.php?act=user_order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
				}
...
function get_agent_orders2($user_id, $num = 10, $start = 0, $condition)
{
	/* 取得订单列表 */
	$arr    = array();
	...
	*/
	...
	$sql = "SELECT agent_parent_id, order_id, order_sn, order_status, shipping_id, shipping_status, pay_status, add_time, " .
			"(goods_amount) AS total_fee ".
			" FROM " .$GLOBALS['ecs']->table('order_info') .
			" WHERE agent_parent_id = '$user_id' and  {$q}  ORDER BY add_time DESC";
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
...
function get_iuser_orders2($user_id, $num = 10, $start = 0, $condition)
{
	/* 取得订单列表 */
	$arr    = array();
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		if ($row['order_status'] == OS_UNCONFIRMED)
		{
			$row['handler'] = "<a href=\"huo15user.php?act=agent_affirm&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('确认要审核通过该订单吗？')) return false;\">审核订单</a>";
		}
		else if ($row['order_status'] == OS_SPLITED)
		{
			/* 对配送状态的处理 */
			if ($row['shipping_status'] == SS_SHIPPED)
			{
				@$row['handler'] = "<a href=\"huo15user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
			}
			elseif ($row['shipping_status'] == SS_RECEIVED)
			{
				@$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
			...
			else
			{
				if ($row['pay_status'] == PS_UNPAYED)
				{
					@$row['handler'] = "<a href=\"huo15user.php?act=agent_order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
				}
				else
				{
					@$row['handler'] = "<a href=\"huo15user.php?act=agent_order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
				}
 ...
 * @return  array       $order_list     订单列表
 */
function get_agent_orders_condition($user_id,$condition, $num = 10, $start = 0)
{
	/* 取得订单列表 */
	$arr    = array();
	$q = '';
 ...
 * @return  bool        $bool
 */
function agent_unpay($order_id, $user_id = 0, $action_note)
{
	$arr = array();
	/* 查询订单信息，检查状态 */
	...
	$f = update_order($order_id, $arr);

	/* 记录log */
	order_action($order['order_sn'], OS_CONFIRMED, $order['shipping_status'], PS_UNPAYED, $action_note);
	return $f;
 ...
 * @return  bool        $bool
 */
function agent_pay($order_id, $user_id = 0, $action_note)
{
	$arr = array();
	/* 查询订单信息，检查状态 */
	$sql = "SELECT * FROM " .$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
