...
if (in_array($action, $ui_arr))
{
    ...
    $smarty->assign('page_title', "代理中心"); // 页面标题
...
if ($action == 'default')
{
	...
	//获取代理商业务总金额
	$sql = "select SUM(goods_amount) from " . $ecs->table('order_info') . " where agent_parent_id = " . $user_id;
	$agent_money_sum = $db->getOne($sql);
	$smarty->assign('agent_money_sum', $agent_money_sum);
	
	//获取代理商业务未付款订单金额
	$sql = "select SUM(goods_amount) from " . $ecs->table('order_info') . " where agent_parent_id = " . $user_id . " and pay_status = 0 and order_status < 2";
	$agent_unpay_sum = $db->getOne($sql);
	$smarty->assign('agent_unpay_sum', $agent_unpay_sum);
	
	//获取代理商业务已经付款订单金额
	$sql = "select SUM(goods_amount) from " . $ecs->table('order_info') . " where agent_parent_id = " . $user_id . " and pay_status = 2 and shipping_status = 0";
	$agent_pay_sum = $db->getOne($sql);
	$smarty->assign('agent_pay_sum', $agent_pay_sum);
	
	//获取代理商业务已经发货订单金额
	$sql = "select SUM(goods_amount) from " . $ecs->table('order_info') . " where agent_parent_id = " . $user_id . " and pay_status = 2  and order_status = 5 and is_ship_ok = 1";
	$agent_done_sum = $db->getOne($sql);
	$smarty->assign('agent_done_sum', $agent_done_sum);
	
	//获取代理商业务扫码订单金额
	$sql = "select SUM(goods_amount) from " . $ecs->table('order_info') . " where agent_parent_id = " . $user_id . " and pay_status = 2  and order_status = 5 and is_ship_ok = 0";
	$agent_scan_sum = $db->getOne($sql);
	$smarty->assign('agent_scan_sum', $agent_scan_sum);
	
	
	//获取垂直业务订单数量
	$sql = "select COUNT(*) from " . $ecs->table('huo15_order_info') . " where agent_parent_id = " . $user_id;
	$user_order_num = $db->getOne($sql);
	$smarty->assign('user_order_num', $user_order_num);
	
	//获取垂直业务发货未完结订单数量
	$sql = "select COUNT(*) from " . $ecs->table('huo15_order_info') . " where agent_parent_id = " . $user_id. " and pay_status = 2  and order_status = 5 and is_ship_ok = 0";
	$user_order_scan_num = $db->getOne($sql);
	$smarty->assign('user_order_scan_num', $user_order_scan_num);
	
	//获取垂直业务已发货订单数量
	$sql = "select COUNT(*) from " . $ecs->table('huo15_order_info') . " where agent_parent_id = " . $user_id. " and pay_status = 2  and order_status = 5 and is_ship_ok = 1";
	$user_order_end_num = $db->getOne($sql);
	$smarty->assign('user_order_end_num', $user_order_end_num);
	...
	$total_agent = sizeof($query_arr);
	
	//总下级人数
	$smarty->assign('total_agent', $total_agent);
	//代理信息
	$agent_list = array();
	foreach ($dealer_level as $k => $v) {
		$sql_huo15 = "SELECT count('user_id') AS sum FROM " . $ecs->table('huo15_agent') . " WHERE parent_id= " . $_SESSION['user_id'] . " AND agent_brand='".$k."' AND is_ok = 1" ;
		$res = $db->getRow($sql_huo15);
		$agent_list[$k]['sum'] = $res['sum'];
		
	}
	
	$sql = "SELECT COUNT(user_id) AS sum FROM " . $ecs->table('huo15_agent') . " WHERE parent_id= " . $_SESSION['user_id'] . " AND is_ok=0" ;
	if (!$total_vertifing = $db->getOne($sql)) {
		//show_message('系统错误，请联系火一五信息科技有限公司（15288986891）修复');
	}
	$smarty->assign('agents', $agent_list);
	$smarty->assign('total_vertifing', $total_vertifing);
...
elseif ($action == 'act_login')
{
  ...
  if($login_type==3)
  {
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        show_message($_LANG['login_success'] . $ucdata , array($_LANG['back_up_page'], "我的代理中心"), array($back_act,'distribute.php'), 'info');
    }
    else
    {
        $_SESSION['login_fail'] ++ ;
...
elseif ($action == 'update_agent_do') {
	...
	foreach ($_POST as $k => $v) {
		if (preg_match('/agent_brand(.*)/', $k, $match)) {
			$tmp[$match[1]]['agent_brand'] = isset($v) ? htmlspecialchars(trim($v)) : '';
				
		}
		elseif (preg_match('/agent_level(.*)/', $k, $match)) {
			$tmp[$match[1]]['agent_level'] = isset($v) ? htmlspecialchars(trim($v)) : '';
	
		}
		elseif (preg_match('/ok(.*)/', $k, $match)) {
			$tmp[$match[1]]['is_ok'] = isset($v) ? htmlspecialchars(trim($v)) : '';
	
		}
		elseif (preg_match('/start_date(.*)/', $k, $match)) {
			$tmp[$match[1]]['start_date'] = isset($v) ? htmlspecialchars(trim($v)) : '';
		
		}
		elseif (preg_match('/end_date(.*)/', $k, $match)) {
			$tmp[$match[1]]['end_date'] = isset($v) ? htmlspecialchars(trim($v)) : '';
	...
	$temp = array();
	foreach ($tmp as $k => $v) {
		$v['user_id'] = trim($_POST['id']);
		$v['parent_id'] = trim($_SESSION['user_id']);
		$temp[$k] = $v; 
	}
	
	$update_agent = $temp;//更新huo15_agent表数据
	
	$arrCheck = array();
	foreach ($update_agent as $k => $v) {
		$arrCheck[] = $v['agent_brand'];
	}
	if(sizeof(array_unique($arrCheck)) != sizeof($arrCheck)) {
		show_message('代理品牌填写重复，请重新审核确认！');
	}
	...
	foreach ($update_agent as $k => $v) {
		//检查等级是否符合要求
		$sql = "select agent_level from " . $ecs->table('huo15_agent') . " where user_id = " . $_SESSION['user_id'] . " and agent_brand = '" . $v['agent_brand'] . "'";
		$upLevel = $db->getOne($sql);
		if ($upLevel >= $v['agent_level']) {
			show_message('下级代理级别不能比您的级别（'.$upLevel.'级）高，请重新审核。');
		}
		$sql = "select COUNT(*) from " . $ecs->table('huo15_agent') . " where user_id = " . $v['user_id'] . " and agent_brand = '" . $v['agent_brand'] . "'";
		$query_flag = $db->getOne($sql);
		
		if ($query_flag) {
			$where = " agent_brand = '" . $v['agent_brand'] . "' and user_id = " . $v['user_id'];
			
			if ($res = $db->autoExecute($ecs->table('huo15_agent'), $v, 'UPDATE', $where)) {
			   //show_message('更新代理商信息表成功！');
			} else {
				show_message('更新代理商信息主表出问题，请联系火一五信息科技解决');
			}
		} else {
			//检查等级是否符合要求
			$sql = "select agent_level from " . $ecs->table('huo15_agent') . " where user_id = " . $_SESSION['user_id'] . " and agent_brand = 'not'";
			$upLevel = $db->getOne($sql);
			if ($upLevel >= $v['agent_level']) {
				show_message('下级代理级别不能比您的级别（'.$upLevel.'级）高，请重新审核。');
			}
			$where = " agent_brand = 'not' and user_id = " . $v['user_id'];
	...
	show_message('更新成功', 0, '/mobile/distribute.php');
...
elseif ($action == 'update_agent') {
	...
	$sql_huo15 = 'SELECT is_ok, agent_brand, agent_level, start_date, end_date FROM '.$ecs->table('huo15_agent'). ' WHERE user_id = '.$user[user_id] . " and parent_id = " . $_SESSION['user_id'];
	...
	//将下来选项添加到下级代理品牌代理里面 :$t 可以授权的代理信息
	$tmp = array();
	foreach ($user['brand_info'] as $k => $v) {
		foreach ($t as $key => $val) {
			if ($val['agent_brand'] == $v['agent_brand']) {
				$v['option'] = $val['option'];
				$tmp[$k] = $v;
	...
	$smarty->assign('arrBAndL', $t);//可以授权的代理信息
...
elseif ($action == 'act_is_ok_btn') {
	
	if (empty($_SESSION['user_id'])) {
		show_message('非法操作！清先登录');
	}
	
	//判断审核状态

	$is_ok = intval(trim($_GET['is_ok']));
	
	if ($is_ok == 1) {
		
		$sql = "DELETE FROM " . $ecs->table('huo15_agent') . " WHERE user_id = " . $_GET['user_id'];
		if (!$db->query($sql)) {
			show_message('删除申请代理用户失败，请联系火一五信息科技技术人员解决！');
		} else {
			show_message('删除成功', '返回分销中心', "http://" . $_SERVER['SERVER_NAME'] . "/mobile/distribute.php");
		}
		
	} else {
		$sql = "UPDATE " . $ecs->table('huo15_agent') . "SET is_ok=1  WHERE user_id = " . $_GET['user_id'];
		if (!$db->query($sql)) {
			show_message('审核申请代理用户失败，请联系火一五信息科技技术人员解决！');
		} else {
			show_message('审核成功！', '返回分销中心', "http://" . $_SERVER['SERVER_NAME'] . "/mobile/distribute.php");
		}
	}
}
elseif ($action == 'vertifing') {
	//默认展示审核中的用户
	$brand = $_GET['brand'] ? trim($_GET['brand']) :'';
	$user_id = intval(trim($_SESSION['user_id']));
	//如果显示的是全体未审核用户
	if (!$brand) {
		$smarty->assign('huo15_vertifing_agent', 1);
		$sql = "SELECT h.user_id, u.user_name, h.agent_level, h.agent_brand, h.is_ok FROM " . $ecs->table('huo15_agent') . " AS h, " . $ecs->table('users') . "AS u WHERE h.user_id = u.user_id AND h.parent_id = " . $user_id . " AND is_ok = 0";
		$arr = $db->getAll($sql);
		...
		foreach ($arr as $v) {
			$temp = user_info($v['user_id']);
			$temp['agent_level'] = $v['agent_level'];
			$temp['agent_brand'] = $v['agent_brand'];
			$agent_arr[] = $temp;
		}
		
		
		//p($agent_arr);die;
		$smarty->assign('arr_info', $agent_arr);
	} else {//审核过的用户
		
		$sql = "SELECT h.user_id, u.user_name, h.agent_level, h.agent_brand, h.is_ok FROM " . $ecs->table('huo15_agent') . " AS h, " . $ecs->table('users') . "AS u WHERE h.user_id = u.user_id AND h.parent_id = " . $user_id . " AND h.is_ok =1 AND h.agent_brand = '" . $brand . "'";
		$arr = $db->getAll($sql);
	...
	$smarty->assign('action', 'vertifing');
	$smarty->display('distribute.dwt');
}
...
elseif ($action =='generate') {
	$u = trim($_GET['u']) ? trim($_GET['u']) : 0;
	if (preg_match('/[A-Za-z]/', $u)) {
		show_message('黑客攻击！');
	}
	$sql = "select * from " . $ecs->table('huo15_agent') . " where user_id = " . $u;
	...
	$smarty->assign('u', $u);
	/*
	 $u = intval(trim($_GET['u']));
	//获取代理品牌、代理等级
	$sql = "SELECT agent_brand, agent_level, is_ok FROM " . $ecs->table('huo15_agent') . " WHERE user_id = " . $u;
	$arr = $db->getAll($sql);
	...
	foreach ($arr as $v) {
		...
		if ($v['is_ok']) {
			...
			$str_options .= "<option value='".$v['agent_brand']."'>".$v['agent_brand']."</option>";
		}
	...
	foreach ($dealer_level as $k => $v) {
		foreach ($arr as $a) {
			if ($a['agent_brand'] == $k) {
				$tmp[$k]['level'] = $a['agent_level'];
				$tmp[$k]['brand'] = $k;
				$tmp[$k]['num'] = sizeof($v['item']);
			} else {
				//$tmp[$k]['level'] = '0';
			}
		}
	...
	$smarty->assign('_u', $u);
	$smarty->assign('brands', $arr);
	$smarty->assign('str_options', $str_options);
	*/
	
	$smarty->assign('action', 'generate');
	$smarty->display('distribute.dwt');
}
//显示分销会员的分层订单
