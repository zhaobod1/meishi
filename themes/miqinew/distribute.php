...
elseif ($action == 'update_agent_do') {
	...
	//huo15修改用户表 更新huo15_agent表
	/*
	//清除原始信息
	$sql = "DELETE FROM " . $ecs->table('huo15_agent') . " WHERE user_id=".$user_id;
	
	if ($res = $db->query($sql)) {
	
	} else {
		show_message('更新huo15_agent表失败，请与火一五信息科技联系解决问题');
	}
	*/
	...
	show_message('更新成功', 0, '/mobile/distribute.php?act=vertifing');
