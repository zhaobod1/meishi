...
//获取代理商头像
$sql = "select content from " . $ecs->table('reg_extend_info') . " where reg_field_id = 15 and user_id = " . trim($_SESSION['user_id']);
$imgUrl = $db->getOne($sql);
if ($imgUrl) {
	$smarty->assign('imgUrl', 'WEB_PATH'.$imgUrl);
}
/* 如果是显示页面，对页面进行相应赋值 */
if (in_array($action, $ui_arr))
{
...
if ($action == 'yq') {
	if(empty($_SESSION['user_id'])) {
		show_message('清先登录注册，然后再加入代理商', 0, 'user.php?act=login');
	}
	...
	$sql = "SELECT agent_level FROM " . $ecs->table('huo15_agent') . " WHERE user_id ="  .$parent_id;
	$parent_level = $db->getRow($sql);
	$parent_level = intval($parent_level['agent_level']);
	if ($parent_level >= $level) {
		$level = $parent_level + 1;
	}
	$sql = "SELECT id FROM " .$ecs->table('huo15_agent') . " WHERE user_id = " . $_SESSION['user_id'] . " AND agent_brand = '" . $brand . "'";
	$arr = $db->getRow($sql);
	
	if ($arr) {
		echo '<script>alert("已经申请过了哦！")</script>';
	} else {
		$sql = "INSERT INTO " . $ecs->table('huo15_agent') . " (user_id, parent_id, agent_level, agent_brand) VALUES(".$_SESSION['user_id'].", ".$parent_id.", ".$level.",'".$brand."')";
		if($db->query($sql)) {
			echo '<script>alert("申请成功！请耐心等待管理员审核")</script>';
		} else {
			echo '<script>alert("申请失败！请联系管理员")</script>';
		}
	}
...
if ($action == 'default')
{
	...
	if($rank_points>$level_register_up||$rank_points==$level_register_up){
		...
		$tianxin="代理商：否     <a href='./'>点此链接申请</a>";
		$tianxinflag="";
	}
	*/
	
	$dealer_level = unserialize($GLOBALS['_CFG']['dealer_level']);
	$sql_huo15 = "SELECT is_ok, agent_level, agent_brand, parent_id FROM " . $ecs->table('huo15_agent') . " WHERE user_id = " . $user_id;
	$tianxin = '';//标示文字
	
	if ($query_arr = $db->getAll($sql_huo15)) {
		
		foreach ($query_arr as $v) {
			
			if ($v['is_ok'] == '0') {
				$tianxin .=  "<span style='color:red'>" .$v['agent_brand'] . "</span>：代理资格审核中<br/>";
			} else {
				foreach ($dealer_level as $key => $val) {
					if ($key == $v['agent_brand']) {
						$v['agent_name'] = $val['item'][$v['agent_level']]['name'];
					}
				}
				$tianxin .= "<span style='color:red'>" .$v['agent_brand'] . "</span>:<span style='color:red'>" . $v['agent_name'] . "</span><br/>";
				$tianxinflag=1;
			}
			$smarty->assign('is_ok',        $tianxinflag);
		}
	} else {
		$tianxin="代理商：否     <a href='./'>点此链接申请</a>";
	}
...
elseif($action == 'checkout') {
	...
	ecs_header("Location: user.php?act=consignee_edit\n");
	exit;
}
...
elseif($action == 'consignee_edit') {
	...
	$smarty->assign('action', 'consignee_edit');
	$smarty->display('user_clips.dwt');
}
//  授权证书
elseif($action == 'certification') {
	$user_id = isset($_SESSION['user_id']) ? trim($_SESSION['user_id']) : null;
	if ($user_id) {
		$sql = "SELECT is_ok, agent_level, agent_brand, parent_id, start_date, end_date FROM " . $ecs->table('huo15_agent') . " WHERE user_id = " . $user_id;
	...
	foreach ($agent_arr as $k => $v) {
		$v['name'] = $extend_info_list[1]['content'];
		$v['phone'] = $extend_info_list[0]['content'];
		$v['wx'] = $extend_info_list[3]['content'];
		if ($v['agent_brand'] == 'AL') {

			$v['no'] = "AL" . str_pad(trim($v['agent_level']), 2, '0', STR_PAD_LEFT) . str_pad(trim($user_id), 6, '0', STR_PAD_LEFT);
		
		} else {
			$v['no'] = "MSG" . str_pad(trim($v['agent_level']), 2, '0', STR_PAD_LEFT) . str_pad(trim($user_id), 6, '0', STR_PAD_LEFT);
			
		}
		
		if ($user_name != 'admin') {
			$v['start_date'] = substr($v['start_date'],0,4) . " 年 " . substr($v['start_date'],5,2) . " 月 " . substr($v['start_date'],8,2) . " 日 ";
			$v['end_date'] = substr($v['end_date'],0,4) . " 年 " . substr($v['end_date'],5,2) . " 月 " . substr($v['end_date'],8,2) . " 日 ";
	...
	$smarty->assign('action', 'certification');
...
if ($action == 'register')
{
	$user_id = isset($_GET['u']) ? intval(trim($_GET['u'])) : null;
	//$apply_brand = isset($_GET['brand']) ? trim($_GET['brand']) : null;
	//$apply_level = isset($_GET['level']) ? trim($_GET['level']) : null;
	
	if (!$user_id || preg_match('/[A-Za-z]/', $user_id)) {
		show_message('非法申请！请重新联系代理商或者管理员！');
		
	}
	$sql = "select * from " . $ecs->table('huo15_agent') . " where user_id = " . $user_id;
	$arrAgent = $db->getRow($sql);
	if ($is_ok = trim($arrAgent['is_ok']) == '0') {
		show_message('非法操作，代理商还没有审核');
	}
...
elseif ($action == 'act_register')
{
    ...
    else
    {
		...
		//如果是已经注册的用户
		$user_id = isset($_POST['id']) ? trim($_POST['id']) : 27;
     
        //扩展信息 火一五信息科技
        if (register($username, $password, $email, $other) !== false)
        {
        	
        	$sql = "SELECT user_id FROM " . $ecs->table('users') . "ORDER BY user_id DESC LIMIT 1";
        	$last_id = $db->getOne($sql);
        	
        	if ($last_id) {
        		
        		//$user_id = isset($_POST['user_id']) ? intval(trim($_POST['user_id'])) : null;
        		//$apply_brand = isset($_POST['apply_brand']) ? trim($_POST['apply_brand']) : null;
        		//$apply_level = isset($_POST['apply_level']) ? trim($_POST['apply_level']) : null;
        		/*
        		//检验是否合法
        		$config =  get_dealer_level();
        		$level_info = array();
        		...
        		if (!$level_info) {
        			...
        			$sql = "DELETE FROM " . $ecs->table('users') . " WHERE user_name='" .$username ."'";
        			if (!$db->query($sql)) {
        				show_message('更新数据库出错');
        			}
        			show_message('申请品牌不正确');
        		}
        		...
        		$count_level = sizeof($level_info['item']);
        		
        		$parent_level = get_parent_level($user_id,$ecs,$db);
        		
        		if ($apply_level > $count_level || $apply_level <=$parent_level) {
        			...
        			show_message('申请等级不合法');
        		}
        		
        		
        		*/
        		$field_values['user_id'] = intval(trim($last_id));
        		//$field_values['agent_level'] = $apply_level;
        		//$field_values['agent_brand'] = $apply_brand;
        		$field_values['parent_id'] = $user_id;
        		$field_values['is_ok'] = 0;
        
        		if(!$db->autoExecute($ecs->table('huo15_agent'), $field_values)) {
        			$sql = "DELETE FROM " . $ecs->table('users') . " WHERE user_name='" .$username ."'";
        			if (!$db->query($sql)) {
        				show_message('更新数据库出错');
        			}
        			show_message('录入出错，请联系管理员，稍后再试');
        			$sql = "DELETE FROM " . $ecs->table('users') . " WHERE user_name='" .$username ."'";
        ...
        else
        {
        		...
        		$last_id = $db->getOne($sql);
        		//检测是否已经到达可以代理品牌的数量
        		$sql = "select COUNT(*) from " . $ecs->table('huo15_agent') . " where user_id = (" .
        				"select user_id from" . $ecs->table('users') . " where user_name = '" . $username .
        				"')" ;
        		$brand_num = $db->getOne($sql);//现在拥有的代理品牌数量
        		$agents = get_dealer_level();
        		if (intval($brand_num) < sizeof($agents)) {
        			$field_values['user_id'] = intval(trim($last_id));
        			//$field_values['agent_level'] = $apply_level;
        			//$field_values['agent_brand'] = $apply_brand;
        			$field_values['parent_id'] = $user_id;
        			...
        			if(!$db->autoExecute($ecs->table('huo15_agent'), $field_values)) {
        				...
        				show_message('申请成功！请等待审核');
        			}
        		} else {
        			show_message('您已经申请代理了全部品牌，无需申请');
        		}
...
elseif ($action == 'act_login')
{
  ...
  if($login_type==3)
  {
			  ...
			  else
			  {
				include_once(ROOT_PATH . 'include/lib_passport.php');
					...
					if (register($cu_user_name, $password, $email, $other) !== false)
					{
						...
						$ucdata = empty($user->ucdata)? "" : $user->ucdata;
						show_message(sprintf($_LANG['login_success'], $cu_user_name . $ucdata), array($_LANG['back_up_page'], '代理商中心'), array($back_act, 'distribute.php'), 'info');
					}
					else
					{
						$_SESSION['login_fail'] ++ ;
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        show_message($_LANG['login_success'] . $ucdata , array($_LANG['back_up_page'], '代理商中心'), array($back_act,'distribute.php'), 'info');
    }
    else
    {
        $_SESSION['login_fail'] ++ ;
...
elseif ($action == 'add_goods_done')
{
	...
	if (isset($_POST['good0'])) {
		...
		foreach ($_POST as $k => $v) {
			...
			if (preg_match('/good/', $k)) {
				...
				$sql = "select order_id from " . $ecs->table('order_info') . " where order_sn = '" . $v['order_sn'] . "'";
	...
	$order_goods = agent_priceAndMinNum($temp, $temp[0]['user_id'], $ecs, $db);
	$field_values = array();
	foreach ($order_goods as $k => $v) {
		...
		$sql = "select COUNT(*) from " . $ecs->table('order_goods') . " where goods_id = " . $field_values['goods_id'] . " and order_id = " . $field_values['order_id'];
		$is_ok = $db->getOne($sql);
		if (!$is_ok) {
			if (!$db->autoExecute($ecs->table('order_goods'), $field_values)) {
				$res['err_msg'] = "添加订单失败，请重试！";
				$res['result'] = 0;
				die($json->encode($res));
			}
		} else {
			$sql = "select goods_number from " . $ecs->table('order_goods') . " where goods_id = " . $field_values['goods_id'] . " and order_id = " . $field_values['order_id'];
			$nums = $db->getOne($sql);
			...
			if (!$db->autoExecute($ecs->table('order_goods'), $field_values, 'update', $where)) {
				$res['err_msg'] = "添加订单失败，请重试！";
				$res['result'] = 0;
				die($json->encode($res));
			}
	...
	//更新订单金额
	$sql = "select goods_number, goods_price from " . $ecs->table('order_goods') . " where order_id = " . $order_goods[0]['order_id'];
	$arr = $db->getAll($sql);
	$total = 0;
	foreach ($arr as $k => $v) {
		$total += floatval($v['goods_number']) * floatval($v['goods_price']);
	}
	$sql = "update " . $ecs->table('order_info') . " set goods_amount = " . $total . " where order_id = " . $order_goods[0]['order_id'];
	$db->query($sql);
	$res['err_msg'] = "提交订单成功！";
	$res['result'] = 1;
	$res['order_id'] = $order_goods[0]['order_id'];
	die($json->encode($res));
}
/* 代理商添加订单操作  火一五信息科技 */
elseif ($action == 'agent_add_order_do')
{
	include_once('include/cls_json.php');
	if (isset($_POST['user_id'])) {
		...
		$order_info = array();
		$order_info['user_id'] = json_str_iconv(trim($_POST['user_id']));
		$order_info['order_sn'] = json_str_iconv(trim($_POST['order_sn']));
		$order_info['consignee'] = json_str_iconv(trim($_POST['consignee']));
		$order_info['country'] = json_str_iconv(trim($_POST['country']));
		$order_info['province'] = json_str_iconv(trim($_POST['province']));
		$order_info['city'] = json_str_iconv(trim($_POST['city']));
		$order_info['district'] = json_str_iconv(trim($_POST['district']));
		$order_info['address'] = json_str_iconv(trim($_POST['address']));
		$order_info['tel'] = json_str_iconv(trim($_POST['tel']));
		$order_info['agent_parent_id'] = json_str_iconv(trim($_POST['session_id']));
		...
		$order_info['postscript'] = json_str_iconv(trim($_POST['postscript']));
		$sql = "select COUNT(*) from " . $ecs->table('order_info') . " where order_sn = '" . $order_info['order_sn'] . "'";
		...
		if ($db->autoExecute($ecs->table('order_info'), $order_info)) {
			$res['err_msg'] = "添加订单成功！";
			$res['result'] = 1;
			die($json->encode($res));
		}
...
elseif ($action == 'agent_add_order')
{
	...
	if (isset($_POST['user_id'])) {
		$json = new JSON();
		$res = array('err_msg' => '', 'result' => '');
		$_POST['user_id'] = json_str_iconv(trim($_POST['user_id']));
		
		$consignee = get_consignee($_POST['user_id']);
		$consignee_list = get_consignee_list($_POST['user_id']);
		$smarty->assign('name_of_region',   array($_CFG['name_of_region_1'], $_CFG['name_of_region_2'], $_CFG['name_of_region_3'], $_CFG['name_of_region_4']));
		...
		foreach ($consignee_list as $region_id => $consignee)
		{
			...
			$consignee['country_name'] = get_regions_name($consignee['country']);
			...
			$temp[$region_id] = $consignee;
		}
		$consignee_list = $temp;
		$res['result'] = $consignee_list;
		die($json->encode($res));
	}
	...
	/*huo15显示下级会员数量*/
	$sql_huo15 = "SELECT user_id, agent_level, agent_brand, parent_id FROM " . $ecs->table('huo15_agent') . " WHERE parent_id= " . $_SESSION['user_id'];
	$query_arr = $db->getAll($sql_huo15);
	$temp = array();
	foreach ($query_arr as $k => $v) {
		$sql = "select user_name from " . $ecs->table('users') . " where user_id = " . $v['user_id'];
		$v['user_name'] = $db->getOne($sql);
		$temp[$k] = $v;
		
	}
	$k = sizeof($temp);
	$temp[$k]['user_name'] = '选择代理商';
	$temp[$k]['user_id'] = 'n';
	$query_arr = $temp;
	//获取商品分类
	...
	$temp = array();
	foreach ($pcat_array as $k => $v) {
		
		$sql = "select goods_id, goods_name from " . $ecs->table('goods') . " where cat_id = " . $v['id'];
	...
	//$goods_list = get_agent_priceAndMinNum($goods_list,'', $ecs, $db);
	
	$smarty->assign('goods_list', $goods_list);
	
	
	$smarty->assign('agent_list', $query_arr);
	
	$smarty->assign('action', 'agent_add_order');
	$smarty->display('user_transaction_huo15.dwt');

}
/* 垂直业务添加订单  火一五信息科技 */
elseif ($action == 'user_add_order')
{
	include_once('include/cls_json.php');
	include_once(ROOT_PATH . 'include/lib_transaction.php');
	include_once('include/lib_clips.php');
	...
	$smarty->assign('agent_list', $query_arr);
	$smarty->assign('real_goods_count', 1);
	$smarty->assign('action', 'user_add_order');
	$smarty->display('user_transaction_huo15.dwt');

}
/* 查看代理商订单列表 火一五信息科技 */
elseif ($action == 'agent_order_list')
{
	 include_once(ROOT_PATH . 'include/lib_transaction.php');
	if (!$user_id) {
		show_message('请先登录，再进行订单处理！', '返回登录页面', "user.php");
	 ...
	 $pager  = get_pager('user.php', array('act' => 'order_list'), $record_count, $page);
	 $orders = get_agent_orders($user_id, $pager['size'], $pager['start']);
	 //p($orders);die;
	 $merge  = get_user_merge($user_id);
	 $smarty->assign('merge',  $merge);
    $smarty->assign('pager',  $pager);
    $smarty->assign('orders', $orders);
    $smarty->assign('action', 'agent_order_list');
    $smarty->display('user_transaction_huo15.dwt');
	 
}
/* 查看代理商未付款订单列表 火一五信息科技 */
elseif ($action == 'agent_unpay_list')
{
	include_once(ROOT_PATH . 'include/lib_transaction.php');
	if (!$user_id) {
		show_message('请先登录，再进行订单处理！', '返回登录页面', "user.php");
	...
	$smarty->assign('orders', $orders);
	$smarty->assign('action', 'agent_order_list');
	$smarty->assign('flag', 'unpay');
	$smarty->display('user_transaction_huo15.dwt');

}
/* 查看代理商已经发货订单列表 火一五信息科技 */
elseif ($action == 'agent_done_list')
{
	include_once(ROOT_PATH . 'include/lib_transaction.php');
	if (!$user_id) {
		show_message('请先登录，再进行订单处理！', '返回登录页面', "user.php");
...
/* 查看代理商发货未完成订单列表 火一五信息科技 */
elseif ($action == 'agent_scan_list')
{
	include_once(ROOT_PATH . 'include/lib_transaction.php');
	if (!$user_id) {
		show_message('请先登录，再进行订单处理！', '返回登录页面', "user.php");
	...
	$smarty->assign('pager',  $pager);
	$smarty->assign('orders', $orders);
	$smarty->assign('flag', 'scan');
	$smarty->assign('action', 'agent_order_list');
	$smarty->display('user_transaction_huo15.dwt');

}

/* 查看代理商已经付款订单列表 火一五信息科技 */
elseif ($action == 'agent_pay_list')
{
	include_once(ROOT_PATH . 'include/lib_transaction.php');
	if (!$user_id) {
		show_message('请先登录，再进行订单处理！', '返回登录页面', "user.php");
...
elseif ($action == 'agent_async_order_list')
{
	...
	$start = $_POST['last'];
	$limit = $_POST['amount'];
	if(preg_match('/unpay/', $_GET['flag'])) {
		$condition = array(
				'pay_status' => 0,
				
		);
		$orders = get_agent_orders2($user_id, $limit, $start, $condition);
		
	} elseif (preg_match('/pay/', $_GET['flag'])) {
		$condition = array(
				'pay_status' => 2,
				'shipping_status' => 0
		);
	...
	} elseif (preg_match('/scan/', $_GET['flag'])) {
		$condition = array(
				'pay_status' => 2,
				'order_status' => 5,
				'is_ship_ok'   => 0
	...
	} elseif (preg_match('/done/', $_GET['flag'])) {
		$condition = array(
			'pay_status' => 2,
			'order_status' => 5,
			'is_ship_ok'   => 1
	...
	//垂直订单列表 全部
	elseif (preg_match('/user_order_list/', $_GET['flag'])) {
		$condition = array(
				'pay_status' => 2,
				'order_status' => 5,
				//'is_ship_ok'   => 1
	...
	if(is_array($orders)){
		foreach($orders as $vo){
			...
			$tracking = ($vo['shipping_id'] > 0) ? '<a href="user.php?act=order_tracking&order_id='.$vo['order_id'].'" class="c-btn3">扫码发货</a>':'';
			$asyList[] = array(
					'order_status' => '订单状态：'.$vo['order_status'],
					'order_handler' => $vo['handler'],
					'order_content' => '<a href="user.php?act=agent_order_detail&order_id='.$vo['order_id'].'"><table width="100%" border="0" cellpadding="5" cellspacing="0" class="ectouch_table_no_border">
            <tr>
...
elseif ($action == 'huo15_shipping')
{
	...
	$count_no = 0;
	//获取订单商品数目
	$sql = "select SUM(goods_number) from " . $ecs->table('order_goods') . " where order_id = " . $order_id;
	$goodsNumber = $db->getOne($sql);
	
	//获取已经发货数目
	$sql = "select COUNT(*) from " . $ecs->table('huo15_instance') . " where no_order like  '%" . $order_id . "%'";
	$shipedNumber = $db->getOne($sql);
	if ($shipedNumber == $goodsNumber) {
		$sql = "update " . $ecs->table('order_info') . " set is_ship_ok = 1 where order_id = " . $order_id;
		if (!$db->query($sql)) {
			show_message('数据出错，请联系火一五信息科技技术人员，QQ：645612509');
		}
		$sql = "update " . $ecs->table('order_info') . " set is_ship_num_ok = 1 where order_id = " . $order_id;
		if (!$db->query($sql)) {
			show_message('数据出错，请联系火一五信息科技技术人员，QQ：645612509');
		}
	}
	foreach ($_POST as $k => $v) {
		if ($k == 'order_id') {
			...
			if($huo15NO == "C") {//箱码
				...
				if ($okInsert) {
					//判断是否存在了订单
					if (!preg_match('/'.$order_id.'/', $arr['no_order'])) {
						//每箱个数
						...
						$field_values['no_order'] = $query_no_order . $order_id . ',';
				...
				if ($okInsert) {
					...
					if (!preg_match('/'.$order_id.'/', $arr['no_order'])) {
						$field_values['no_order'] = $arr['no_order'] . $order_id . ',';
	...
	//把商品下发给下级代理商
		$sql = "SELECT goods_id, user_id FROM ".$ecs->table('order_goods')." as g INNER JOIN ".$ecs->table('order_info')." as i ON i.order_id = g.order_id  WHERE  g.order_id = 377 AND i.is_ship_ok = 1 AND g.is_ship_num_ok = 0";
		
		if($arrGoodsIds = $db->getAll($sql)){
			
			foreach ($arrGoodsIds as $v) {
				
				//检测是否存在商品
				$sql = "select COUNT(goods_id) from " . $ecs->table('huo15_agent_goods_info') . "where goods_id = " . $v['goods_id'];
				
				if ($f = $db->getOne($sql)) {
					
					$sql = "update " . $ecs->table('huo15_agent_goods_info') . " set goods_num = goods_num + " . $goodsNumber . " where goods_id = " . $v['goods_id'] . " and user_id = " . $v['user_id'];
					
					if (!$db->query($sql)) {
						show_message('更新下级代理商商品库存失败， 请重试');
					}
					//更改下发货完成状态
					$sql = "update " . $ecs->table('order_goods') . " set is_ship_num_ok = 1 where order_id = " . $order_id;
					if (!$db->query($sql)) {
						show_message('更新下级代理商商品发货完成状态失败， 请重试');
					}
						
				} else {
					
					$sql = "insert into " . $ecs->table('huo15_agent_goods_info') . "  (goods_num, goods_id, user_id) value( goods_num + " . $goodsNumber . ", ".$v['goods_id'].", ".$v['user_id']." )";
...
/* 追溯do huo15 */
elseif ($action == 'trace_do')
{
	$code = isset($_POST['code']) ? trim($_POST['code']) : 0;
	
	if ($code) {
		$huo15NO = substr($code, 0, 1);
		
		if($huo15NO == "C") {//箱码
		
			//获取数据库编码箱子
			$no_case = ltrim(substr($code, -6, 6), '0');
			//获取对应箱子的订单单号
			$sql = "select no_order from " . $ecs->table('huo15_instance') . " where no_case = ".$no_case." limit 1";
			$query_no_order = $db->getOne($sql);
			//获取对应的商品ID
			...
			$arrOrder = explode(',', $query_no_order);
			$traceName = array();
			...
			if (!empty($v)) {
					...
					if (preg_match('/U/', $order_id)) {
						
						$sql = "select consignee, add_time from " . $ecs->table('huo15_order_info') . " where order_id = " . substr($order_id, 1);
						$x = $db->getRow($sql);
						
							
						$traceName[$k][] = '垂直业务：' . $x['consignee'];
						$traceName[$k][] = date('Y-m-d H:i:s', $x['add_time']);
					} else {
						//$sql = "select consignee from " . $ecs->table('order_info') . " where order_id = " . $order_id;
						$sql = "select agent_parent_id, add_time from " . $ecs->table('order_info') . " where order_id = " . $order_id;
						$x = $db->getRow($sql);
						if (!$x) {
							$traceName[$k][] = '订单丢失';
							$traceName[$k][] = date('Y-m-d H:i:s');
						} else {
							$sql = "select user_name from " . $ecs->table('users') . " where user_id = " . $x['agent_parent_id'];
								
							$traceName[$k][] = $db->getOne($sql);
							$traceName[$k][] = date('Y-m-d H:i:s', $x['add_time']);
						}
			...
			die(json_encode($traceName));
		
		} else {//普通编码
			//获取普通编码的ID号
			$no_id = ltrim(substr($code, -6, 6), '0');
			
			//获取数据库编码箱子，订单单号
			$sql = "select  no_order from " . $ecs->table('huo15_instance') . " where id = " . $no_id;
			$arr = $db->getOne($sql);
			$arrOrder = explode(',', $arr);
...
elseif ($action == 'operate') {
	...
	//付款操作
	if (isset($_POST['pay']) && isset($_POST['order_id'])) {
		if (agent_pay($_POST['order_id'], $user_id, $action_note)) {
			show_message('订单已经修改为付款状态', '返回订单列表', 'user.php?act=agent_order_list');
		}
	}
	//取消付款操作
	if (isset($_POST['unpay']) && isset($_POST['order_id'])) {
		if (agent_unpay($_POST['order_id'], $user_id, $action_note)) {
			show_message('订单已经修改为未付款状态', '返回订单列表', 'user.php?act=agent_order_list');
		}
	}
	
	
	//一键发货
	if (isset($_POST['to_shipping']) && isset($_POST['order_id'])) {
		$invoice_no = empty($_REQUEST['invoice_no']) ? '' : trim($_REQUEST['invoice_no']);  //快递单号
		
		agent_to_shipping($invoice_no, $_POST['order_id'], $action_note, $ecs, $db);
	}
	
	
	//设置为未发货
	if (isset($_POST['unship']) && isset($_POST['order_id'])) {
	...
	if (isset($_POST['cancel']) && isset($_POST['order_id'])) {
		...
		if(update_order($_POST['order_id'], array('order_status' => OS_CANCELED, 'to_buyer' => $cancel_note))) {
			show_message('订单已经修改为取消状态', '返回订单列表', '/user.php?act=agent_order_list');
	...
	if (isset($_POST['confirm']) && isset($_POST['order_id'])) {
		...
		if(update_order($_POST['order_id'], array('order_status' => OS_CONFIRMED, 'to_buyer' => $cancel_note))) {
			show_message('订单审核成功', '返回订单列表', 'user.php?act=agent_order_list' );
	...
	if (isset($_POST['remove']) && isset($_POST['order_id'])) {
		...
		if(update_order($_POST['order_id'], array('order_status' => OS_CONFIRMED, 'to_buyer' => $cancel_note))) {
			...
			/* 删除订单 */
			$db->query("DELETE FROM ".$ecs->table('order_info'). " WHERE order_id = '$_POST[order_id]'");
			$db->query("DELETE FROM ".$ecs->table('order_goods'). " WHERE order_id = '$_POST[order_id]'");
			$db->query("DELETE FROM ".$ecs->table('order_action'). " WHERE order_id = '$_POST[order_id]'");
			$action_array = array('delivery', 'back');
			del_delivery($order['order_id'], $action_array);
			
			/* todo 记录日志 */
			admin_log($order['order_sn'], 'remove', 'order');
			show_message('订单已经删除', '订单列表', 'user.php?act=agent_order_list');
...
elseif ($action == 'agent_order_detail')
{
	...
	/* 订单详情 */
	//$order = get_order_detail($order_id, $user_id);
	$order = get_agent_order_detail($order_id, $user_id);
	...
	$smarty->assign('action',      'agent_order_detail');
	$smarty->assign('order',      $order);
	$smarty->assign('goods_list', $goods_list);
	$smarty->display('user_transaction_huo15.dwt');
}
...
/* huo15 审核订单，确认 */
elseif ($action == 'agent_affirm')
{
	include_once(ROOT_PATH . 'include/lib_transaction.php');

	$order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;

	if (agent_affirm($order_id, $user_id))
	{
		ecs_header("Location: user.php?act=agent_order_list\n");
		exit;
	}
	else
	{
		$err->show($_LANG['order_list_lnk'], 'user.php?act=agent_order_list');
	}
}
/* 确认收货 */
elseif ($action == 'affirm_received')
{
    ...
    {
       // $err->show($_LANG['order_list_lnk'], 'user.php?act=agent_order_list');
    	$err->show('确认收货需要您的下级代理商进行操作，您不能进行此项操作！点击返回订单列表', 'user.php?act=agent_order_list');
    }
}
...
elseif ($action == 'update_user')
{
	...
	else
	{
		...
		show_message($_LANG['username_invalid'], 0, $links);
		//        $user['sex']            = 0;
		//        $user['pay_points']     = 0;
		//        $user['rank_points']    = 0;
		//        $user['user_money']     = 0;
	...
	$sql_huo15 = 'SELECT is_ok, agent_brand, agent_level FROM '.$ecs->table('huo15_agent'). ' WHERE user_id = '.$user[user_id];
	$arr_query = $db->getAll($sql_huo15);
	
	$user['brand_info'] = $arr_query;
	if (sizeof($user['brand_info'])) {
		$smarty->assign('agent_level_OK', 1);
	}
	//获取代理商可以授权的品牌和等级
	
	$sql = "select agent_level, agent_brand from " . $ecs->table('huo15_agent') . " where user_id = " . $_SESSION['user_id'] . " and is_ok = 1";
	$arrBAndL = $db->getAll($sql);
	//获取代理信息
	$arrAgentInfo = get_dealer_level();
	
	$t = array();
	foreach ($arrBAndL as $k => $v) {
		if (is_null($k)) $k = '0';
	
		foreach ($arrAgentInfo as $key => $val) {
			if ($key == $v['agent_brand']) {
				$v['end_num'] = sizeof($val['item']);
				$v['start_num'] = intval(trim($v['agent_level']));
				$s = '';
				if ($v['end_num'] != $v['start_num']) {
					for ($i=$v['start_num']+1;$i<$v['end_num'];$i++) {
...
/* 修改订单金额 */
elseif ($action == 'updat_order_amount') {
	$order_id = isset($_POST['order_id']) ? intval(trim($_POST['order_id'])) : null;
	$order_amount = isset($_POST['order_amount']) ? floatval(trim($_POST['order_amount'])) : 0;
	$sql = "select goods_amount from " . $ecs->table('order_info') . " where order_id = " . $order_id;
	if ($order_id) {
		if ($db->autoExecute($ecs->table('order_info'), array('goods_amount' => $order_amount), 'UPDATE', " order_id = " . $order_id)) {
			$sql = "select goods_amount from " . $ecs->table('order_info') . " where order_id = " . $order_id;
			$amount = $db->getOne($sql);
			die('{"amount":"'.$amount.'","msg":"修改成功！"}');
		} else {
			die('{"amount":"-1","msg":"修改失败！"}');
		}
	}
}

/* 更新用户个人信息  huo15*/
elseif ($action == 'update_agent_do')
{
	include_once(ROOT_PATH . '/include/cls_image_tianxin.php');
	$image = new cls_image($_CFG['bgcolor']);
	
	/* 检查图片：如果有错误，检查尺寸是否超过最大值；否则，检查文件类型 */
	if (isset($_FILES['extend_field15']['error'])) // php 4.2 版本才支持 error
	{
		// 最大上传文件大小
		$php_maxsize = ini_get('upload_max_filesize');
		$htm_maxsize = '1M';
		// 照片
		if ($_FILES['extend_field15']['error'] == 0)
		{
			if (!$image->check_img_type($_FILES['extend_field15']['type']))
			{
				show_message($_LANG['invalid_goods_img'], 1, array(), false);
			}
			elseif ($_FILES['extend_field15']['error'] == 1)
			{
				show_message(sprintf($_LANG['goods_img_too_big'], $php_maxsize), 1, array(), false);
			}
			elseif ($_FILES['extend_field15']['error'] == 2)
			{
				show_message(sprintf($_LANG['goods_img_too_big'], $htm_maxsize), 1, array(), false);
			}
		}
		/* 处理商品图片 */
		$goods_img        = '';  // 初始化图片
		...
		// 如果上传了商品图片，相应处理
		if ($_FILES['extend_field15']['tmp_name'] != '' && $_FILES['extend_field15']['tmp_name'] != 'none')
		{
			/* 删除原来的图片文件 */
			if ($_POST['id'] > '0') {
				$sql = "SELECT  content " .
						" FROM " . $ecs->table('reg_extend_info') .
						" WHERE user_id = '$_POST[id]' AND reg_field_id = 15";
				$row = $db->getOne($sql);
				if ($row != '' && is_file('../' . $row)) {
					@unlink('../' . $row);
				}
			...
			$original_img   = $image->upload_image($_FILES['extend_field15']); // 原始图片
			 
			if ($original_img === false)
			{
				show_message($image->error_msg(), 1, array(), false);
			}
	...
	$_POST['extend_field15'] = $goods_img;
	...
	if ($db->autoExecute($ecs->table('users'), $other, 'UPDATE', "user_name = '$username'")) {
		/* 提示信息 */
		
		show_message('更新成功', 0, '/mobile/user.php');
	}
