...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    foreach ($level_info as $v) {
    	if ($v['brand_id'] == $goods['brand_id']) {
    		$goods['agent_level'] = $v['agent_level'];
    	}
    }
    //p($goods);die;
    if (!isset($goods['agent_level'])) {
    	show_message('请先登录系统！', '代理商入口', 'user.php');
    }
	//获取代理价格和启货量
	$sql = "select min_num, price from " . $ecs->table('huo15_gdextend') . " where goods_id = " . $goods['goods_id'] . " and level=" . $goods['agent_level'];
	if (!$db->query($sql)) {
		show_message('请先登录系统！');
	}
	$tmp = $db->getRow($sql);
    $goods['agent_price'] = $tmp['price'];
    $goods['min_num'] = $tmp['min_num'];
   // p($goods);die;
    if ($goods === false)
    {
        /* 如果没有找到任何记录则跳回到首页 */
