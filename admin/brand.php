...
$root_path = str_replace('mobile/', '', ROOT_PATH);
include_once($root_path. 'includes/cls_image.php');
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    assign_query_info();
   // $return_arr = get_str_options($ecs, $db, $_REQUEST['id']);
   // $level_name = $return_arr['level_name'];
   // $str_options = $return_arr['str_options'];
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    /* 处理图片 */
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $param = "brand_name = '$_POST[brand_name]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]', level_name = '{$level_name}'  ";
    if (!empty($img_name))
    {
        //有图片上传
...
/**
 * 火一五信息科技 获取代理等级设置名称列表
 * @return string
 */
function get_str_options($ecs, $db, $id='') {
	//获取代理设置
	$sql = 'SELECT `level_name` FROM ' . $ecs->table('brand') . ' WHERE `brand_id` = ' . $id;
	
	$level_name = $db->getRow($sql);
	$level_name = $level_name['level_name'];
	//$level_options = array();//选项数组
	$str_options = '';//格式化选项html
	if (1) {
		$level_select = get_dealer_level();
		foreach ($level_select as $k => $v) {
			//$level_options[] = $k;
			 
			if ($k != $level_name)
				$str_options = $str_options . '<option value="' . $k . '" >' . $k . '</option>';
		}
		 
		 
	}
	if ($level_name) {
		$str_options = $str_options . '<option value="0" >不设代理（隐藏）</option>';
	}
	$arr['str_options'] = $str_options;
