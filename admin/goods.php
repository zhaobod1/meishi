...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $tmp = array();
  	foreach ($goods_list['goods'] as $k => $v ) {
  		...
  		//huo15 库存
  		$sql = "select COUNT(*) from " . $ecs->table('huo15_instance') . " where good_id = ".$v['goods_id']."  and  no_order is null";
  		$v['huo15_goods_number'] = $db->getOne($sql);
  		$tmp[$k] = $v;
  		//检测虚拟库存
  		$sql = "select goods_number from " . $ecs->table('goods') . " where goods_id = " . $v['goods_id'];
  		if ($db->getOne($sql) < '10000') {
  			$sql = "update " . $ecs->table('goods') . " set goods_number = 100000" ;
  			$db->query($sql);
  		}
  	}
  	
  	$goods_list['goods'] = $tmp;
...
elseif ($_REQUEST['act'] == 'no_generate' ) {
	
	
	$good_id = trim($_GET['good_id']);
	$sql = "SELECT COUNT(*) FROM " . $ecs->table('huo15_instance') . "WHERE good_id=" . $good_id;
	$total = $db->getOne($sql); 
	$smarty->assign('total', $total);
	$sql = "SELECT COUNT(DISTINCT no_case) FROM " . $ecs->table('huo15_instance') . "WHERE good_id=" . $good_id;
	$total_case = $db->getOne($sql);
	$smarty->assign('total_case', $total_case);
	//产品编号
	$smarty->assign('good_id', $good_id);
	
	$smarty->display('generate_info.htm');
}
elseif ($_REQUEST['act'] == 'no_details') {
	$good_id = intval(trim($_REQUEST['good_id']));
	
	$sql = "SELECT COUNT(*) FROM ecs_huo15_instance WHERE good_id=" . $good_id;
	
	$total = $db->getOne($sql);
	
	include ROOT_PATH . 'admin/includes/page.class.php';

	$page = new Page($total, 15000);
	
	$sql="select * from ecs_huo15_instance where good_id=".$good_id."  {$page->limit}";
	$no_list = $db->getAll($sql);
	$tmp = array();
	$sql = "SELECT goods_sn FROM " . $ecs->table('goods') . "WHERE goods_id=" . $good_id;
	$good_sn = $db->getOne($sql);
	
	foreach ($no_list as $k => $v) {
		if ($v['no_case'] != $test) {
			$v['generate_case_no'] = 'C' .  strtoupper($good_sn) . $v['date'] . str_pad($v['no_case'], 6, '0',  STR_PAD_LEFT);
			$test = $v['no_case'];
		}
		$v['generate_no'] = strtoupper($good_sn) . $v['date'] . str_pad($v['id'], 6, '0', STR_PAD_LEFT);
		
		$tmp[$k] = $v;
	}
	$test = $no_list[0]['no_case'];
	$temp[0]['generate_case_no'] = 'C' .  strtoupper($good_sn) . $no_list[0]['date'] . str_pad($no_list[0]['no_case'], 6, '0',  STR_PAD_LEFT);
	
	$no_list = $tmp;
	//p($no_list);die;
	$fpage = $page->fpage();
	$smarty->assign('fpage', $fpage);
	$smarty->assign('no_list', $no_list);
	$smarty->assign('full_page',    1);
	$smarty->assign('good_id', $good_id);
	$smarty->assign('is_nos_list', 1);
	$smarty->display('no_list.htm');
}
elseif ($_REQUEST['act'] == 'generate_confirm') {
	
	$good_id = trim(intval($_POST['good_id']));
	$case_num = trim(intval($_POST['case_num']));
	$goods_num = trim(intval($_POST['goods_num']));
	$minus_num = trim(intval($_POST['minus_num']));
	
	if (!$case_num && !$goods_num && !$minus_num) {
		sys_msg('不能全为空！请填写数据！');
	}
	if (($case_num <=0 || $goods_num <=0) && !$minus_num) {
		sys_msg('每箱数量填写不合法,或者批量增加数量填写不合法！');
	}
	$is_add = $goods_num || $case_num ;
	$is_minus = !!$minus_num;
	
	if ($is_add && $is_minus) {
		sys_msg('生成编码和减去产品编码操作不能同时进行，请重新操作！');
	}
	
	if ($is_add) {
		//$cases = $goods_num / $case_num;
		
		$sql = "SELECT * FROM  " . $ecs->table('huo15_instance') . "  ORDER BY id DESC LIMIT 1 ";
		$arr = $db->getAll($sql);
		//$last_id = intval($arr[0]['id']) ? intval($arr[0]['id']) : 0;
		$last_case = intval($arr[0]['no_case']) ? intval($arr[0]['no_case']) : 0;
		$date = date('Ymd');
		
		for ($i=1;$i<=$goods_num;$i++) {
			$temp_num = ceil($i / $case_num);
			$no_case = $last_case + $temp_num;
			$sql = "INSERT INTO " . $ecs->table('huo15_instance') . " (date, no_case, good_id) VALUES ('".$date."', ".$no_case." ,".$good_id.")";
			
			if (!$db->query($sql)) {
				sys_msg('系统数据库出错，请稍后再试！');
			}
		}
		$href[0]['text'] = '直接返回';
		$href[0]['href'] = 'goods.php?act=no_generate&good_id='.$good_id;
		sys_msg('编码生成成功！', 0, $href, true);
		
	} elseif ($is_minus) {
		
		$sql = "SELECT COUNT(*) FROM " . $ecs->table('huo15_instance') . "WHERE good_id=" . $good_id;
		$last_no = intval($db->getOne($sql));
		$sql = "SELECT no_order FROM " . $ecs->table('huo15_instance') . "WHERE id = " . $last_no;
		if ($db->getOne($sql)) {
			$href[0]['text'] = '直接返回';
			$href[0]['href'] = 'goods.php?act=no_generate&good_id='.$good_id;
			sys_msg('已经发货不可以删除！', 0, $href, true);
			die;
		}
		
		$sql = "DELETE FROM " . $ecs->table('huo15_instance') . "WHERE good_id=".$good_id." ORDER BY id DESC LIMIT " . $minus_num;
		if (!$db->query($sql)) {
			sys_msg('系统数据库出错，请稍后再试！');
		}
		/*
		$sql = "SELECT COUNT(*) FROM " . $ecs->table('huo15_instance');
		$sum = intval($db->getOne($sql)) + 1;
		$sql = "ALTER TABLE " . $ecs->table('huo15_instance') . " AUTO_INCREMENT=" . $sum;
		if (!$db->Query($sql)) {
			sys_msg('系统数据库出错，请稍后再试！');
		}
		*/
...
//var_dump($img_list);exit;
        ...
        //获取等级价格、最少拿货量信息
        $sql = "SELECT * FROM " . $ecs->table('huo15_gdextend') . " WHERE goods_id=" . $_GET['goods_id'];
        $arr_huo15 = $db->getAll($sql);
        $smarty->assign('level_price_info', $arr_huo15);
    ...
    //p($_GET);
   	if ($_GET['goods_id']) {
   		$query_good_id = $_GET['goods_id'];
   	} else {
   		$query_good_id = 0;
   	}
   
    $sql = "SELECT brand_id FROM " . $ecs->table('goods') . " WHERE goods_id=" . $query_good_id;
    if ($brand_id) {
    ...
    $sql = "SELECT level_name FROM " . $ecs->table('brand') . " WHERE brand_id=" . $brand_id;
    $level_name = $db->getOne($sql);
   
    //配置信息
    $config_level = get_dealer_level();
    //价格等级个数
    $num_level = array();
    ...
    //添加新产品时候的等级个数
    $num_new = array();
    foreach ($config_level as $k => $v) {
    	if ($k == $level_name) {
    		foreach ($v['item'] as $key => $value) {
    			$num_level[$key] = $value['name'];
    			
    		}
    		
    		$num_level['sum'] = sizeof($v['item']);
    		
    	}
    	//p($k . ' , ' . $level_name );
    	$num_new[$k] = sizeof($v['item']);
    }
    
    //p($num_new_max);die;
    $num_new_max = max($num_new);
    //p($num_new_max);die;
    if (!$num_new_max) {
    	$smarty->assign('is_update', 1);
    }
    //添加新产品的最大等级数
    $smarty->assign('num_new_max', $num_new_max);
    
    //修改商品时，产品的等级数量
    $smarty->assign('num_level', $num_level);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	} while ($_POST['huo15_l' . $count_i] != '');
	//查询是否合法
	if ($_POST['brand_id']) {
		$dealer_level = get_dealer_level();
		
		$sql = "SELECT level_name FROM " . $ecs->table('brand') . " WHERE brand_id=" . $_POST['brand_id'];
		$level_name = $db->getOne($sql);
		
		foreach ($dealer_level as $k => $v) {
			if ($k == $level_name) {
				if (sizeof($v['item']) < $count_i) {
					sys_msg($level_name . ':只能最多填写' . sizeof($v['item']) . "级代理。请重新添加");
				}
			}
		}
	}
	...
	$qty = isset($_POST['qty']) ? intval($_POST['qty']) : '50';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (qty, goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ($qty, '$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
    ...
    if ($_REQUEST['act'] == 'insert')
    {
    	...
    	//插入商品等级价格表 huo15_gdextend
    	
    	
    	for ($i=0;$i<$count_i;$i++) {
    		$field_values = array();
    		$field_values['goods_id'] = $goods_id;
    		$field_values['level'] = $i;
    		$field_values['min_num'] = intval($_POST['huo15_min_num' . $i]);
    		$field_values['price'] = floatval($_POST['huo15_l' . $i]);
    		//p($field_values);die;
    		if ($db->autoExecute('ecs_huo15_gdextend', $field_values)) {
    			//删除商品信息并显示错误信息
    			//code huo15
    
    			//sys_msg('插入等级价格成功！');
    		} else {
    			$sql = "DELETE FROM " . $ecs->table('huo15_gdextend') . "WHERE goods_id=" . $goods_id;
    			$db->query($sql);
    			$sql = "DELETE FROM " . $ecs->table('goods') . "WHERE goods_id=" . $goods_id;
    			$db->query($sql);
    			sys_msg('插入等级价格失败，请联系火一五信息科技！');
    		}
    	}
    } else {//更新扩展价格库
    	
    	 
    	//更新商品等级价格表 huo15_gdextend
    	for ($i=0;$i<$count_i;$i++) {
    		$field_values = array();
    		$field_values['goods_id'] = $goods_id;
    		$field_values['level'] = $i;
    		...
    		$field_values['price'] = floatval($_POST['huo15_l' . $i]);
    		//p($field_values);die;
    		$where = "goods_id = " . $goods_id . " and level = " . $i;
    		if ($db->autoExecute('ecs_huo15_gdextend', $field_values, 'UPDATE', $where)) {
    			//删除商品信息并显示错误信息
    			//code huo15
