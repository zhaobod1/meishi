...
if ($_REQUEST['act'] == 'list')
{
	...
	foreach($user_list1 as $aa){
		...
		$sql = "SELECT count('uid')  FROM ecs_users  where level_parent_id='$user_id' ";
		$number = $db->query($sql);
		$number =mysql_fetch_array($number);
		$aa['number']=$number[0];
		$sql_huo15 = 'SELECT parent_id, agent_level, agent_brand, is_ok FROM '.$ecs->table('huo15_agent'). ' WHERE user_id='.$user_id;
		$arr_query = $db->getAll($sql_huo15);
		$aa['agent_info'] = $arr_query;
		
		
		//下级代理商个数
		foreach ($aa['agent_info'] as $v) {
			$sql_huo15 = "SELECT count('user_id') FROM ecs_huo15_agent WHERE parent_id=".$user_id;
			
			$number_h = $db->query($sql_huo15);
			$number_h = mysql_fetch_array($number_h);
			$aa['number'] = $number_h[0];
		}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $huo15['dealer_level_no'] = isset($_POST['dealer_level_no']) ? htmlspecialchars(trim($_POST['dealer_level_no'])) : '';
    $huo15['agent_brand'] = isset($_POST['agent_brand']) ? htmlspecialchars(trim($_POST['agent_brand'])) : '';
    ...
    /* 火一五信息科技 更新huo15_agent表 */
    for ($i=0;$i<2;$i++) {
    	$arr_query[$i]['agent_brand'] = isset($_POST['agent_brand'  . $i]) ? htmlspecialchars(trim($_POST['agent_brand'  . $i])) : '';
    	$arr_query[$i]['agent_level'] = isset($_POST['agent_level'  . $i]) ? htmlspecialchars(trim($_POST['agent_level'  . $i])) : '';
    	$arr_query[$i]['is_ok'] = isset($_POST['is_ok'  . $i]) ? htmlspecialchars(trim($_POST['is_ok'  . $i])) : '';
    ...
    foreach ($arr_query as $k => $v) {
    	if ($v['agent_brand'] !='') {
    		$keys = array_keys($dealer_level);
    		if (!in_array($v['agent_brand'], $keys)) {
    
    			die("'".$v['agent_brand']."':输入的品牌不正确");
    		}
    		 
    		if (intval($v['agent_level']) > sizeof($dealer_level[$v['agent_brand']]['item'])) {
    			die($v['agent_brand'].'最大能发展"'.sizeof($dealer_level[$v['agent_brand']]['item']).'个级别代理');
    
    		}elseif(intval($v['agent_level'])<0) {
    			$v['agent_level'] = -1;
    		}
    
    		$v['user_id'] = $user_id;
    		
    		if ($res = $db->autoExecute($ecs->table('huo15_agent'), $v)) {
    ...
    foreach ($arr_query as $k => $v) {
    	if (!$v['agent_brand']) {
    		...
    		$v['user_id'] = $user_id;
    		if ($res = $db->autoExecute($ecs->table('huo15_agent'), $v, 'INSERT')) {
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql_huo15 = "SELECT user_id FROM " . $ecs->table('huo15_agent') ."WHERE user_id = '" . $_GET['id'] . "'";
    if ($db->query($sql_huo15)) {
    	$sql_huo15 = "DELETE FROM " . $ecs->table('huo15_agent') ."WHERE user_id = '" . $_GET['id'] . "'";
    	if (!$db->query($sql_huo15)) {
    		echo '更新数据表失败';
    	}
    }
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, agent_brand, dealer_level_no, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
